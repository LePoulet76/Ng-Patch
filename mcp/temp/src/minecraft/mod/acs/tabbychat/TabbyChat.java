...
public class TabbyChat {
   ...
   private void spamCheck(String _chan, List<ChatLine> lastChat) {
      ...
      if(theChan.chatLog.size() < lastChat.size()) {
         ...
         for(last = 0; last < _size; ++last) {
            newChat = newChat + ((ChatLine)lastChat.get(last)).func_74538_a();
            if(ClientProxy.clientConfig.enableTimestamp) {
               oldChat = ((ChatLine)theChan.chatLog.get(last)).func_74538_a().replaceAll("^" + ((TimeStampEnum)this.generalSettings.timeStampStyle.getValue()).regEx, "") + oldChat;
            } else {
               oldChat = ((ChatLine)theChan.chatLog.get(last)).func_74538_a() + oldChat;
            }
         }
         ...
         if(oldChat.equals(newChat) && !oldChat.equalsIgnoreCase("\u00a7r") && lastChat.size() != 0) {
            ...
            if(var10 instanceof TaggableChatLine) {
               ...
               theChan.chatLog.set(0, new TaggableChatLine(var10.func_74540_b(), this.withTimeStamp(var10.func_74538_a()) + " [" + theChan.spamCount + "x]", taggableLast.getTags(), var10.func_74539_c()));
            } else {
               theChan.chatLog.set(0, new ChatLine(var10.func_74540_b(), this.withTimeStamp(var10.func_74538_a()) + " [" + theChan.spamCount + "x]", var10.func_74539_c()));
            }
         } else {
            theChan.hasSpam = false;
            theChan.spamCount = 1;
   ...
   public void pollForUnread(Gui _gui, int _y, int _tick) {
      ...
      try {
         ...
         tickdiff = _tick - ((TaggableChatLine)this.lastChat.get(0)).func_74540_b();
      } catch (Exception var11) {
         return;
      }

      if(tickdiff < 50) {
         float var6 = mc.field_71474_y.field_74357_r * 0.9F + 0.1F;
   ...
   public int processChat(List<ChatLine> theChat) {
      ...
      boolean skip = false;
      int n = theChat.size();
      StringBuilder filteredChat = new StringBuilder(n > 0?((ChatLine)theChat.get(0)).func_74538_a().length() * n:0);

      int coloredChat;
      for(coloredChat = 0; coloredChat < n; ++coloredChat) {
         filteredChat.append(((ChatLine)theChat.get(coloredChat)).func_74538_a());
      }
      ...
      ChatLine var39;
      for(var37 = theChat.iterator(); var37.hasNext(); var38 = var38 + var39.func_74538_a()) {
         var39 = (ChatLine)var37.next();
      }

      cleanedChat = StringUtils.func_76338_a(var38);
   ...
   public void updateButtonLocations() {
      ...
      int vert = sr.func_78328_b() - gnc.chatHeight - 51;
      int horiz = 5;
      int n = this.channelMap.size();

      try {
         if(TabbyChatUtils.is(mc.field_71456_v.func_73827_b(), "GuiNewChatWrapper")) {
            ...
            int dVert = mc.field_71462_r.field_73881_g - 22 - 108;
            int xOff1 = i.getField("posX").getInt(chan) - 3;
            int yOff1 = i.getField("posY").getInt(chan) - dVert;
            horiz += xOff1;
            vert -= yOff1;
            if(gnc.func_73760_d()) {
               ((GuiChatTC)mc.field_71462_r).scrollBar.setOffset(xOff1, yOff1);
            }
         }
      ...
      for(Iterator aHudFld1 = this.channelMap.values().iterator(); aHudFld1.hasNext(); horiz = chan1.getButtonEnd() + 1) {
         ...
         chan1.tab.width(mc.field_71466_p.func_78256_a("<" + chan1.title + ">") + 8);
         ...
         if(chan1.tab == null) {
            ...
            chan1.tab.field_73741_f = chan1.getID();
            chan1.tab.field_73746_c = horiz;
            chan1.tab.field_73743_d = vert;
            chan1.tab.field_73744_e = chan1.getDisplayTitle();
         }
      }
