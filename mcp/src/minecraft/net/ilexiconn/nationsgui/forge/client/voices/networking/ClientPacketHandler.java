...
public class ClientPacketHandler implements IPacketHandler
{
    ...
    private void handleVoiceServerAuthentication(INetworkManager manager, Packet250CustomPayload packet, Player player)
    {
        ...
        if (manager.getSocketAddress() instanceof InetSocketAddress)
        {
            InetSocketAddress bais1 = (InetSocketAddress)manager.getSocketAddress();
            serverAddress = bais1.getAddress().getHostAddress();
        }

        ByteArrayInputStream bais11 = new ByteArrayInputStream(packet.data);
        DataInputStream dis = new DataInputStream(bais11);
        int type = 0;
        String hash = null;
        int udp = 0;
    ...
    private void handleVoiceServer(INetworkManager manager, Packet250CustomPayload packet, Player player)
    {
        ...
        VoiceChat.getProxyInstance().getClientNetwork().startClientNetwork(EnumVoiceNetworkType.values()[type], (String)null, (String)null, 0, maxSoundDistance);
    }
