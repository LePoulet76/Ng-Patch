...
public class ClientSocket {
      ...
      if(ipAndPort == null) {
         ...
         if(targetServer == null || !targetServer.equalsIgnoreCase(ClientProxy.currentServerName)) {
            ...
            try {
               ...
               Minecraft.func_71410_x().func_71373_a(new ServerSwitchExpressGui(ip, port, targetServer));
               ...
               try {
                  ...
                  Minecraft.func_71410_x().func_71373_a(new CustomConnectingGui(new MainGUI(), ip, port, targetServer));
   ...
   public void run() {
      try {
         lastSocketPing = System.currentTimeMillis();
         socket = new Socket("socket.nationsglory.fr", '\ue679');
         ...
         while(in.hasNextLine()) {
            ...
            if(e.startsWith("SUBMITNAME")) {
               serverName = clientName + "##" + Minecraft.func_71410_x().func_110432_I().func_111286_b() + "##" + Utils.toTweaker(MessageDigest.getInstance("SHA-256").digest(Utils.getTweaker().getBytes(StandardCharsets.UTF_8)));
               ...
               if(!e.startsWith("MESSAGE")) {
                  ...
                  if(!e.startsWith("PING_AND_DATA")) {
                     if(e.startsWith("NEED_CONFIRMATION_JOIN_WAITING")) {
                        ...
                        } else if(!(Minecraft.func_71410_x().field_71462_r instanceof WaitingModalGui)) {
                           Minecraft.func_71410_x().func_71373_a(new WaitingModalGui("switch"));
                        }
                        ...
                        if(ClientProxy.isPlayerOnServer() && !ClientProxy.currentServerName.equalsIgnoreCase("hub") && !ClientProxy.currentServerName.equalsIgnoreCase("accueil")) {
                           if(ClientData.isCombatTagged) {
                              Minecraft.func_71410_x().field_71439_g.func_71035_c(I18n.func_135053_a("waiting.combattagged").replaceAll("<server>", serverName));
                              out.println("MESSAGE socket STEPBACK_WAITINGLIST");
                           } else {
                              ClientData.waitingServerIpPort = ipAndPort;
                              if(!(Minecraft.func_71410_x().field_71462_r instanceof WaitingModalGui)) {
                                 Minecraft.func_71410_x().func_71373_a(new WaitingModalGui("join"));
                              }
                           }
                        } else {
                           if(ipAndPort == null) {
