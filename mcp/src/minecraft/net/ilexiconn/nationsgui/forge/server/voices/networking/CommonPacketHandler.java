...
public class CommonPacketHandler implements IPacketHandler
{
    private void handleVoiceData(Packet250CustomPayload packet, Player player, boolean end)
    {
        ...
        VoiceChat.getServerInstance().getVoiceServer().handleVoiceData(player, data, ((EntityPlayerMP)player).entityId, end);
    }

    public void onPacketData(INetworkManager manager, Packet250CustomPayload packet, Player player)
    {
        if (packet.channel.equals("GVC-SMPL") || packet.channel.equals("GVC-SMPLE"))
        {
            this.handleVoiceData(packet, player, packet.channel.equals("GVC-SMPLE"));
        }
    }
}
