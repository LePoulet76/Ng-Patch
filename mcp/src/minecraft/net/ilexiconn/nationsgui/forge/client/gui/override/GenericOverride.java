    ...
    public void renderOverride(Minecraft client, ScaledResolution resolution, float partialTicks)
    {
        if (ClientData.playerInfos != null)
        {
            if (System.currentTimeMillis() - ClientEventHandler.lastPlayerDispayTAB.longValue() >= 50L)
            {
                ...
                {
                    int sizeY = resolution.getScaledHeight();
                    int sizeX = resolution.getScaledWidth();
                    int centerX = sizeX / 2;
                    ...
                    if (displaysCountryEntry() && !NotificationOverride.displaysNotification())
                    {
                        ...
                        if (ClientData.countryTitleInfos.containsKey("relation"))
                        {
                            ...
                            ModernGui.drawRectangle((float)(centerX - 25), 30.0F, this.zLevel, 50.0F, 12.0F);
                            ModernGui.glColorHex(-14869195, 1.0F);
                            ModernGui.drawRectangle((float)(centerX - 25), 42.0F, this.zLevel, 50.0F, 1.0F);
                        }

                        ModernGui.drawScaledStringCustomFont(((String)ClientData.countryTitleInfos.get("countryName")).startsWith("\u00a7") ? I18n.getString("overlay.faction.relation.free") : I18n.getString("overlay.faction.relation.short." + (String)ClientData.countryTitleInfos.get("relation")).toUpperCase(), (float)centerX, 32.0F, 16777215, 0.45F, "center", false, "minecraftDungeons", 23);
