...
public class NGPlayerData
implements IExtendedEntityProperties {
    public static final String ID = "NGPlayerData";
    private static final Map<String, NBTTagCompound> playersData = new HashMap<String, NBTTagCompound>();
    private final EntityPlayer player;
    public List<String> emotes = new ArrayList<String>();
    public List<String> currentEmotes = new ArrayList<String>();
    public List<String> capes = new ArrayList<String>();
    ...
    public static NBTTagCompound listToNBT(List<String> list) {
        ...
        for (String emote : list) {
            nbtList.func_74742_a((NBTBase)new NBTTagString(emote, emote));
        }
        tag.func_74782_a("list", (NBTBase)nbtList);
        return tag;
    }
    ...
    public static List<String> nbtToList(NBTTagCompound props) {
        ...
        for (int i = 0; i < list.func_74745_c(); ++i) {
            if (!(list.func_74743_b(i) instanceof NBTTagString)) continue;
            emotes.add(((NBTTagString)list.func_74743_b((int)i)).field_74751_a);
        }
        return emotes;
    }

    public static final void register(EntityPlayer player) {
        player.registerExtendedProperties(ID, (IExtendedEntityProperties)new NGPlayerData(player));
    }

    public static final NGPlayerData get(EntityPlayer player) {
        if (player != null && ID != null) {
            return (NGPlayerData)player.getExtendedProperties(ID);
        }
        return null;
    }

    public static void saveData(EntityPlayer player) {
        NGPlayerData playerData = NGPlayerData.get(player);
        NBTTagCompound savedData = new NBTTagCompound();
        playerData.saveNBTData(savedData);
        NGPlayerData.storeEntityData(NGPlayerData.getSaveKey(player), savedData);
    }

    public static void loadData(EntityPlayer player) {
        NGPlayerData playerData = NGPlayerData.get(player);
        NBTTagCompound savedData = NGPlayerData.getEntityData(NGPlayerData.getSaveKey(player));
        if (savedData != null) {
    ...
    static NBTTagCompound getEntityData(String name) {
        return playersData.remove(name);
    }

    private static String getSaveKey(EntityPlayer player) {
        return player.getDisplayName() + ":" + ID;
    }
    ...
    public static void refreshPremiumEmotes(String playerName, boolean has) {
        ...
        if (entityPlayer != null) {
            NGPlayerData ngPlayerData = NGPlayerData.get(entityPlayer);
            if (has) {
    ...
    public void saveNBTData(NBTTagCompound compound) {
        ...
        props.func_74782_a("Emotes", (NBTBase)NGPlayerData.listToNBT(this.emotes));
        props.func_74782_a("CurrentEmotes", (NBTBase)NGPlayerData.listToNBT(this.currentEmotes));
        props.func_74782_a("Capes", (NBTBase)NGPlayerData.listToNBT(this.capes));
        ...
        compound.func_74782_a(ID, (NBTBase)props);
    }

    public void loadNBTData(NBTTagCompound compound) {
        try {
            NBTTagCompound props = (NBTTagCompound)compound.func_74781_a(ID);
            this.emotes = NGPlayerData.nbtToList(props.func_74775_l("Emotes"));
            this.currentEmotes = NGPlayerData.nbtToList(props.func_74775_l("CurrentEmotes"));
            this.capes = NGPlayerData.nbtToList(props.func_74775_l("Capes"));
            String string = this.currentCape = props.func_74779_i("CurrentCape").equalsIgnoreCase("none") ? "" : props.func_74779_i("CurrentCape");
    ...
    public boolean addEmote(String emote) {
        if (!this.emotes.contains(emote)) {
            ...
            this.player.func_70006_a(ChatMessageComponent.func_111066_d((String)Translation.get("\u00a74[\u00a7cEmotes\u00a74] \u00a77Vous avez re\u00e7u l'emote '<emote>'").replace("<emote>", emote)));
            this.synchronize();
            return true;
        }
        return false;
    ...
    public boolean addCape(Cape cape) {
        if (!this.capes.contains(cape.getIdentifier())) {
            ...
            this.synchronize();
            this.player.func_70006_a(ChatMessageComponent.func_111066_d((String)Translation.get("\u00a74[\u00a7cCapes\u00a74] \u00a77Vous avez re\u00e7u la cape '<cape>'").replaceAll("<cape>", cape.getDisplayName())));
            return true;
        }
        return false;
    }
    ...
    public List<Cape> getCapes() {
        ...
        for (String capeName : this.capes) {
    ...
    public void onEntityConstructing(EntityEvent.EntityConstructing event) {
        if (event.entity instanceof EntityPlayer && NGPlayerData.get((EntityPlayer)event.entity) == null && ((EntityPlayer)event.entity).getExtendedProperties(ID) == null) {
            NGPlayerData.register((EntityPlayer)event.entity);
        }
    }
    ...
    public void onLivingDeathEvent(LivingDeathEvent event) {
        if (!event.entity.field_70170_p.field_72995_K && event.entity instanceof EntityPlayer) {
            ...
            ((NGPlayerData)event.entity.getExtendedProperties(ID)).saveNBTData(playerData);
            playersData.put(((EntityPlayer)event.entity).field_71092_bJ, playerData);
            NGPlayerData.saveData((EntityPlayer)event.entity);
        }
    }
    ...
    public void onEntityJoinWorldEvent(EntityJoinWorldEvent event) {
        if (!event.entity.field_70170_p.field_72995_K && event.entity instanceof EntityPlayer && ((EntityPlayer)event.entity).field_71092_bJ != null) {
            NBTTagCompound playerData = playersData.remove(((EntityPlayer)event.entity).field_71092_bJ);
            if (playerData != null) {
                ((NGPlayerData)event.entity.getExtendedProperties(ID)).loadNBTData(playerData);
                playersData.remove(((EntityPlayer)event.entity).field_71092_bJ);
            }
            ((NGPlayerData)event.entity.getExtendedProperties(ID)).synchronize();
        }
    }
}
