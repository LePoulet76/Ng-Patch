...
public class ObjectiveOverride extends Gui implements ElementOverride {
   ...
   public void renderOverride(Minecraft client, ScaledResolution resolution, float partialTicks) {
      if(ClientData.versusOverlayData.isEmpty() || ((ArrayList)ClientData.versusOverlayData.get("playersInMatch")).contains(Minecraft.func_71410_x().field_71439_g.field_71092_bJ)) {
         if(System.currentTimeMillis() - ClientEventHandler.lastPlayerDispayTAB.longValue() > 50L && ClientData.currentAssault.isEmpty() && ClientData.currentWarzone.isEmpty() && ClientProxy.serverType.equals("ng") && ClientProxy.clientConfig.displayObjectives && !Minecraft.func_71410_x().field_71474_y.field_74330_P && this.active) {
            int offsetY = 30;
