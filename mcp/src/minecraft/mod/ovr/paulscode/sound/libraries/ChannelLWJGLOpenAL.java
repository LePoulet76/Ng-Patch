    ...
    public boolean preLoadBuffers(LinkedList bufferList)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (int var8 = 0; var8 < bufferList.size(); ++var8)
                {
                    byteBuffer = (ByteBuffer)BufferUtils.createByteBuffer(((byte[])((byte[])bufferList.get(var8))).length).put((byte[])((byte[])bufferList.get(var8))).flip();

                    try
                    {
                        AL10.alBufferData(streamBuffers.get(var8), this.ALformat, byteBuffer, this.sampleRate);
                    }
