...
public class PlayerManagerTransformer implements Transformer
{
    ...
    public void transform(ClassNode node, boolean dev)
    {
        ...
        while (var3.hasNext())
        {
            ...
            if (!methodNode.name.equals(dev ? "filterChunkLoadQueue" : "filterChunkLoadQueue") && !methodNode.name.equals(dev ? "updateMountedMovingPlayer" : "updateMountedMovingPlayer"))
            {
                if (methodNode.name.equals(dev ? "addPlayer" : "addPlayer") || methodNode.name.equals(dev ? "removePlayer" : "removePlayer"))
                {
                    ...
                    for (int target = 0; target < var23; ++target)
                    {
                        ...
                        if (varInsnNode instanceof FieldInsnNode)
                        {
                            ...
                            if (fieldInsnNode1.name.equals(dev ? "playerViewRadius" : "playerViewRadius"))
                            {
            ...
            else
            {
                ...
                for (int lineNumberNode = 0; lineNumberNode < targets; ++lineNumberNode)
                {
                    ...
                    if (patch instanceof FieldInsnNode)
                    {
                        ...
                        if (fieldInsnNode.name.equals(dev ? "playerViewRadius" : "playerViewRadius"))
                        {
