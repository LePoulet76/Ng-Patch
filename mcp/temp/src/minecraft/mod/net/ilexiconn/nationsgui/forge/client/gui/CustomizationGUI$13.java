package net.ilexiconn.nationsgui.forge.client.gui;

import java.io.IOException;
import net.ilexiconn.nationsgui.forge.client.ClientProxy;
import net.ilexiconn.nationsgui.forge.client.gui.CustomizationGUI;
import net.ilexiconn.nationsgui.forge.client.gui.GuiButtonOptionBoolean;

class CustomizationGUI$13 extends GuiButtonOptionBoolean {

   // $FF: synthetic field
   final CustomizationGUI this$0;


   CustomizationGUI$13(CustomizationGUI this$0, int x0, int x1, int x2, int x3, int x4, String x5) {
      super(x0, x1, x2, x3, x4, x5);
      this.this$0 = this$0;
   }

   public void setData(Boolean data) {
      ClientProxy.clientConfig.renderEmotes = data.booleanValue();

      try {
         ClientProxy.saveConfig();
      } catch (IOException var3) {
         var3.printStackTrace();
      }

   }

   public Boolean getData() {
      return Boolean.valueOf(ClientProxy.clientConfig.renderEmotes);
   }

   // $FF: synthetic method
   // $FF: bridge method
   public Object getData() {
      return this.getData();
   }

   // $FF: synthetic method
   // $FF: bridge method
   public void setData(Object var1) {
      this.setData((Boolean)var1);
   }
}
