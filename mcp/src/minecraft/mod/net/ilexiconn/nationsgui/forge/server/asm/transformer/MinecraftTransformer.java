...
public class MinecraftTransformer implements Transformer
{
    ...
    public void transform(ClassNode node, boolean dev)
    {
        ...
        while (var3.hasNext())
        {
            ...
            if (!methodNode.name.equals("startGame") && !methodNode.name.equals("startGame"))
            {
                if (!methodNode.name.equals("loadScreen") && !methodNode.name.equals("loadScreen"))
                {
                    AbstractInsnNode var16;

                    if (!methodNode.name.equals("runTick") && !methodNode.name.equals("runTick"))
                    {
                        if (!methodNode.name.equals("refreshResources") && !methodNode.name.equals("refreshResources"))
                        {
                            if (methodNode.name.equals("screenshotListener") || methodNode.name.equals("screenshotListener"))
                            {
                    ...
                    else
                    {
                        ...
                        for (int var21 = 0; var21 < var22; ++var21)
                        {
                            ...
                            if (var23 instanceof FieldInsnNode)
                            {
                                ...
                                if (var24.getOpcode() == 181 && var24.owner.equals("net/minecraft/entity/player/InventoryPlayer") && var24.name.equals(dev ? "currentItem" : "currentItem"))
                                {
                                    var16 = var23;
                                }
                            }
            ...
            else
            {
                ...
                while (var15.hasNext())
                {
                    ...
                    if (target instanceof MethodInsnNode && ((MethodInsnNode)target).owner.equalsIgnoreCase("net/minecraft/client/Minecraft") && ((MethodInsnNode)target).name.equalsIgnoreCase(dev ? "loadScreen" : "loadScreen") && ((MethodInsnNode)target).desc.equalsIgnoreCase("()V"))
                    {
                        var17 = true;
                    }
                    ...
                    if (var17)
                    {
                        ...
                        var14.add(new MethodInsnNode(183, "net/minecraft/client/Minecraft", dev ? "loadScreen" : "loadScreen", "()V"));
                    }
                }
        ...
        while ((current = current.getNext()) != null)
        {
            if (current.getOpcode() == 16)
            {
                ...
                if (varInsnNode.operand == targetCode)
                {
                    ...
                    patch.add(new FieldInsnNode(180, "net/minecraft/client/settings/KeyBinding", dev ? "keyCode" : "keyCode", "I"));
