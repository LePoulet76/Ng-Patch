...
public class DropdownComponent extends AbstractAssistanceComponent
{
    ...
    public void draw(int mouseX, int mouseY, float partialTicks)
    {
        ...
        this.drawTexturedModalRect(this.posX + this.width - 19, this.posY, 159, 256 + (this.open ? 20 : 0), 20, 19);
        this.drawString(Minecraft.getMinecraft().fontRenderer, this.getDisplayString(), this.posX + 5, this.posY + 6, 16777215);
        ...
        if (this.open)
        {
            for (int i = 0; i < this.choices.size(); ++i)
            {
                ...
                this.drawString(Minecraft.getMinecraft().fontRenderer, (String)this.choices.get(i), this.posX + 5, this.posY + 6 + offset - (1 + i), 16777215);
            }
        }
    }
