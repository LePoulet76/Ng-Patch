...
public enum ServerGUIHandler implements IGuiHandler
{
    ...
    public static HashMap<String, FactionChestInventory> chestInventories;
    ...
    public Object getServerGuiElement(int id, EntityPlayer player, World world, int x, int y, int z) {
        ...
        if (id == NationsGUI.CONFIG.repairMachineID) {
        ...
        if (id == 666) {
            ...
            if (chestInventories.containsKey(RemoteOpenFactionChestPacket.playersTargetGui.get(player.getDisplayName()))) {
                chestInventory = chestInventories.get(RemoteOpenFactionChestPacket.playersTargetGui.get(player.getDisplayName()));
            } else {
                chestInventory = new FactionChestInventory(RemoteOpenFactionChestPacket.playersTargetGui.get(player.getDisplayName()), RemoteOpenFactionChestPacket.playersChestLevel.get(player.getDisplayName()));
                chestInventories.put(RemoteOpenFactionChestPacket.playersTargetGui.get(player.getDisplayName()), chestInventory);
            }
            return new FactionChestContainer(player.field_71071_by, chestInventory, RemoteOpenFactionChestPacket.playersCanTake.get(player.getDisplayName()), RemoteOpenFactionChestPacket.playersCanDeposit.get(player.getDisplayName()), RemoteOpenFactionChestPacket.playersTargetGui.get(player.getDisplayName()));
        }
        if (id == 777) {
