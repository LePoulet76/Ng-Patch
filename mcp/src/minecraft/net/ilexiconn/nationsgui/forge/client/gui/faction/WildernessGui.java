...
extends GuiScreen {
    ...
    public void func_73866_w_() {
        super.func_73866_w_();
        PacketDispatcher.sendPacketToServer((Packet)PacketRegistry.INSTANCE.generatePacket(new FactionWildernessDataPacket()));
    ...
    public void func_73863_a(int mouseX, int mouseY, float par3) {
        ...
        this.drawScaledString(I18n.func_135053_a((String)"faction.search_players.title"), this.guiLeft + 50, this.guiTop + 20, 0x191919, 1.0f, false, false);
        this.drawScaledString(I18n.func_135053_a((String)"faction.search_players.pseudo"), this.guiLeft + 51 + 5, this.guiTop + 50, 0x191919, 0.9f, false, false);
        this.drawScaledString(I18n.func_135053_a((String)"faction.search_players.country"), this.guiLeft + 51 + 80, this.guiTop + 50, 0x191919, 0.9f, false, false);
        this.drawScaledString(I18n.func_135053_a((String)"faction.search_players.power"), this.guiLeft + 51 + 165, this.guiTop + 50, 0x191919, 0.9f, false, false);
        this.drawScaledString(I18n.func_135053_a((String)"faction.search_players.wilderness_filter"), this.guiLeft + 64, this.guiTop + 227, 0x191919, 0.9f, false, false);
        ClientEventHandler.STYLE.bindTexture("faction_search");
        ...
        if (loaded && players.size() > 0) {
            ...
            for (int i = 0; i < players.size(); ++i) {
                if ((!this.wilderness_filter || this.wilderness_filter && players.get(i).get("country").contains("Wilderness")) && (this.searchText.isEmpty() || !this.searchText.isEmpty() && players.get(i).get("name").toLowerCase().contains(this.searchText.toLowerCase()))) {
                    int offsetX = this.guiLeft + 51;
                    Float offsetY = Float.valueOf((float)(this.guiTop + 60 + (i - filterOffsetY) * 20) + this.getSlide());
                    ClientEventHandler.STYLE.bindTexture("faction_search");
                    ModernGui.drawModalRectWithCustomSizedTexture(offsetX, offsetY.intValue(), 51, 60, 245, 20, 512.0f, 512.0f, false);
                    this.drawScaledString(players.get(i).get("name"), offsetX + 5, offsetY.intValue() + 5, 0xFFFFFF, 1.0f, false, false);
                    this.drawScaledString(players.get(i).get("country"), offsetX + 80, offsetY.intValue() + 5, 0xFFFFFF, 1.0f, false, false);
                    this.drawScaledString(players.get(i).get("power") + "/" + players.get(i).get("max_power"), offsetX + 165, offsetY.intValue() + 5, 0xFFFFFF, 1.0f, false, false);
                    ClientEventHandler.STYLE.bindTexture("faction_profil");
                    if (players.get(i).get("unique").equals("true")) {
                        ModernGui.drawModalRectWithCustomSizedTexture(offsetX + 215, offsetY.intValue() + 2, 228, 249, 14, 14, 512.0f, 512.0f, false);
                    }
                    ClientEventHandler.STYLE.bindTexture("faction_search");
                    ...
                    this.hoveredPlayer = players.get(i);
                    continue;
                }
                ++filterOffsetY;
            }
    ...
    private float getSlide() {
        int counter = 0;
        for (int i = 0; i < players.size(); ++i) {
            if (this.wilderness_filter && (!this.wilderness_filter || !players.get(i).get("country").contains("Wilderness")) || !this.searchText.isEmpty() && (this.searchText.isEmpty() || !players.get(i).get("name").toLowerCase().contains(this.searchText.toLowerCase()))) continue;
            ++counter;
        }
    ...
    protected void func_73864_a(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0) {
            ...
            if (this.hoveredPlayer != null && !this.hoveredPlayer.isEmpty()) {
                this.field_73882_e.func_71373_a((GuiScreen)new ProfilGui(this.hoveredPlayer.get("name"), ""));
            }
