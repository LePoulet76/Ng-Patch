...
public class MinimapRequestPacket implements IPacket, IServerPacket
{
    ...
    {
        if (!lastPlayerCall.containsKey(player.username) || System.currentTimeMillis() - ((Long)lastPlayerCall.get(player.username)).longValue() > 15000L)
        {
            lastPlayerCall.put(player.username, Long.valueOf(System.currentTimeMillis()));
            ...
            for (int z = 0; z < this.chunkHeight; ++z)
            {
                ...
                {
                    Chunk chunk = world.getChunkFromChunkCoords(x + chunkX, z + chunkZ);
                        ...
                        for (int z1 = 0; z1 < 16; ++z1)
                        {
                            for (int x1 = 0; x1 < 16; ++x1)
                            {
                                ...
                                do
                                {
                                    ...
                                    if (blockID != 0)
                                    {
                                        MapColor mapcolor = Block.blocksList[blockID].blockMaterial.materialMapColor;
                                        colors[(z * 16 + z1) * this.chunkWidth * 16 + x * 16 + x1] = (byte)(mapcolor.colorIndex * 4);
                                        isTransparent = false;
                                    }
