...
public class NBTTagCompoundTransformer implements Transformer
{
    ...
    public static boolean nbtEquals(NBTTagCompound tag1, Object tag2)
    {
        try
        {
            if (tagMap == null)
            {
                tagMap = NBTTagCompound.class.getDeclaredField("tagMap");
                tagMap.setAccessible(true);
            }

            if (tag1.getTag("rottenTimer") == null && ((NBTTagCompound)tag2).getTag("rottenTimer") == null)
            {
                return ((Map)tagMap.get(tag1)).equals((Map)tagMap.get(tag2));
            }
            else
            {
                NBTTagCompound e = (NBTTagCompound)tag1.copy();
                NBTTagCompound nbtTagCompound2 = (NBTTagCompound)((NBTTagCompound)tag2).copy();
                e.removeTag("rottenTimer");
                nbtTagCompound2.removeTag("rottenTimer");
