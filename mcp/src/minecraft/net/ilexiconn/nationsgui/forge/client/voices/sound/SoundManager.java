...
public class SoundManager
{
    ...
    public void createStream(Datalet data)
    {
        ...
        else
        {
            ...
            if (data.global == 1)
            {
                Entity entity = this.mc.theWorld.getEntityByID(data.id);
                EntityVector vector = (EntityVector)this.entityData.get(Integer.valueOf(data.id));

                if (entity != null)
                {
                    this.mc.sndManager.sndSystem.rawDataStream(universalAudioFormat, false, identifier, (float)entity.posX, (float)entity.posY, (float)entity.posZ, 2, (float)this.voiceChatClient.getSettings().getSoundDistance());
                }
                else if (vector != null)
                {
                    this.mc.sndManager.sndSystem.rawDataStream(universalAudioFormat, false, identifier, (float)vector.x, (float)vector.y, (float)vector.z, 2, (float)this.voiceChatClient.getSettings().getSoundDistance());
                }
                else
                {
                    this.mc.sndManager.sndSystem.rawDataStream(universalAudioFormat, false, identifier, 0.0F, 0.0F, 0.0F, 0, 0.0F);
                }
            }
            else
            {
            ...
            this.mc.sndManager.sndSystem.setPitch(identifier, 1.0F);
            this.mc.sndManager.sndSystem.setVolume(identifier, this.voiceChatClient.getSettings().getWorldVolume());
    ...
    public void replaceStream(Datalet data)
    {
        ...
        this.mc.sndManager.sndSystem.setDistOrRoll(this.generateSource(stream.id), (float)this.voiceChatClient.getSettings().getSoundDistance());
    }
    ...
    public void giveStream(Datalet data)
    {
        ...
        if (!KeyManager.getInstance().isKeyMuted() && stream != null && (!stream.isEntityPlayer() || stream.isEntityPlayer() && !VoiceChat.getProxyInstance().getSettings().isPlayerMuted(((AbstractClientPlayer)stream.entity).username)))
        {
            String identifier = this.generateSource(data.id);
            ...
            {
                this.mc.sndManager.sndSystem.flush(identifier);
                this.mc.sndManager.sndSystem.feedRawAudioData(identifier, stream.buffer.get());
    ...
    private void addStreamSafe(PlayableStream stream)
    {
        if (this.streaming.size() < 3)
        {
            ...
            Entity var71 = this.mc.theWorld.getEntityByID(stream.id);
    ...
    {
        this.mc.sndManager.sndSystem.setPosition(string, posX, posY, posZ);
    }
