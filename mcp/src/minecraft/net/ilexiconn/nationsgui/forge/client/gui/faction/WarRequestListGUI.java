...
public class WarRequestListGUI
extends GuiScreen {
    ...
    public void func_73866_w_() {
        ...
        PacketDispatcher.sendPacketToServer((Packet)PacketRegistry.INSTANCE.generatePacket(new FactionEnemyListPacket((String)FactionGUI.factionInfos.get("id"))));
    ...
    public void func_73863_a(int mouseX, int mouseY, float par3) {
        ...
        if (warsInfos.size() > 0) {
            ...
            for (HashMap<String, Object> warInfos : warsInfos) {
                if (!this.countrySearch.func_73781_b().isEmpty() && !((String)warInfos.get("factionName")).toLowerCase().contains(this.countrySearch.func_73781_b().toLowerCase()) || !this.selectedStatus.equals("all") && !((String)warInfos.get("status")).startsWith(this.selectedStatus)) continue;
                ...
                if (!ClientProxy.flagsTexture.containsKey((String)warInfos.get("factionName"))) {
                    if (!ClientProxy.base64FlagsByFactionName.containsKey((String)warInfos.get("factionName"))) {
                        ...
                        PacketDispatcher.sendPacketToServer((Packet)PacketRegistry.INSTANCE.generatePacket(new FactionGetImagePacket((String)warInfos.get("factionName"))));
                    } else if (ClientProxy.base64FlagsByFactionName.containsKey((String)warInfos.get("factionName")) && !ClientProxy.base64FlagsByFactionName.get((String)warInfos.get("factionName")).isEmpty()) {
                        BufferedImage image = ClientProxy.decodeToImage(ClientProxy.base64FlagsByFactionName.get((String)warInfos.get("factionName")));
                        ClientProxy.flagsTexture.put((String)warInfos.get("factionName"), new DynamicTexture(image));
                    }
                }
                if (ClientProxy.flagsTexture.containsKey((String)warInfos.get("factionName"))) {
                    GL11.glBindTexture((int)3553, (int)ClientProxy.flagsTexture.get((String)warInfos.get("factionName")).func_110552_b());
                    ModernGui.drawScaledCustomSizeModalRect(offsetX + 7, offsetY.intValue() + 3, 0.0f, 0.0f, 156, 78, 27, 15, 156.0f, 78.0f, false);
                }
                this.drawScaledString((String)warInfos.get("factionName"), offsetX + 37, offsetY.intValue() + 7, 0xFFFFFF, 1.0f, false, false);
                ...
                if (mouseX >= offsetX + 202 && mouseX <= offsetX + 202 + 51 && (float)mouseY >= offsetY.floatValue() + 28.0f && (float)mouseY <= offsetY.floatValue() + 28.0f + 15.0f) {
                    ModernGui.drawModalRectWithCustomSizedTexture(offsetX + 202, offsetY.intValue() + 28, 0, 287, 51, 15, 512.0f, 512.0f, false);
                } else {
                    ModernGui.drawModalRectWithCustomSizedTexture(offsetX + 202, offsetY.intValue() + 28, 0, 272, 51, 15, 512.0f, 512.0f, false);
                }
                this.drawScaledString(I18n.func_135053_a((String)"faction.enemy.requests.see"), offsetX + 227, offsetY.intValue() + 32, 0, 1.0f, true, false);
                yOffset += 48;
            }
