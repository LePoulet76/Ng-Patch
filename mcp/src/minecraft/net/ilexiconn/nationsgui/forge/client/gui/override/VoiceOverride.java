...
public class VoiceOverride extends Gui implements ElementOverride
{
    ...
    public void renderOverride(Minecraft client, ScaledResolution resolution, float partialTicks)
    {
        ...
        int y = 97;
        int i = 0;
        Iterator var7 = Minecraft.getMinecraft().theWorld.playerEntities.iterator();

        while (var7.hasNext())
        {
            EntityPlayer ent = (EntityPlayer)var7.next();

            if (ent instanceof AbstractClientPlayer && ClientEventHandler.isPlayerTalk(ent.getCommandSenderName()) && i < 3)
            {
                ++i;
                GL11.glPushMatrix();
                double size1 = 0.8D;
                ...
                int nameWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(ent.getCommandSenderName());
                Gui.drawRect(x + 43, y + 3, x + 43 + nameWidth + 6, y + 3 + 22, -14803651);
                this.drawString(client.fontRenderer, ent.getCommandSenderName(), x + 46, y + 10, 16777215);
                ClientEventHandler.STYLE.bindTexture("voice_card");
                ...
                client.renderEngine.bindTexture(((AbstractClientPlayer)ent).getLocationSkin());
                GL11.glPushMatrix();
        ...
        {
            y += 34;
            this.drawCenteredString(client.fontRenderer, "+" + (i - 3) + " personne(s)", 5, y, 16777215);
        }
    }
}
