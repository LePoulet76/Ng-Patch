...
public class EnterpriseBetGUI extends TabbedEnterpriseGUI
{
    ...
    public void initGui()
    {
        ...
        this.createButton = new GuiButton(0, this.guiLeft + 10, this.guiTop + 165, 100, 20, I18n.getString("enterprise.bet.btnCreate"));
        ...
        this.betInput = new GuiTextField(this.fontRenderer, this.guiLeft, this.guiTop, 54, 10);
        this.betInput.setEnableBackgroundDrawing(false);
        this.betInput.setMaxStringLength(6);
    }
    ...
    public void drawScreen(int mouseX, int mouseY)
    {
        ...
        if (!this.createButton.enabled && mouseX > this.guiLeft + 10 && mouseX < this.guiLeft + 10 + 100 && mouseY > this.guiTop + 165 && mouseY < this.guiTop + 165 + 20)
        {
            tooltipToDraw = I18n.getString("enterprise.bet.btnCreate.disabled");
        }

        this.drawScaledString(I18n.getString("enterprise.bet.title").replace("#enterprise#", (String)EnterpriseGui.enterpriseInfos.get("name")), this.guiLeft + 131, this.guiTop + 15, 1644825, 1.4F, false, false);

        if (loaded)
        {
            this.drawScaledString((String)betsInfos.get("stats_active"), this.guiLeft + 203 - this.fontRenderer.getStringWidth((String)betsInfos.get("stats_active")), this.guiTop + 37, 16777215, 1.0F, false, false);
            this.drawScaledString((String)betsInfos.get("stats_reward"), this.guiLeft + 292 - this.fontRenderer.getStringWidth((String)betsInfos.get("stats_reward")), this.guiTop + 37, 16777215, 1.0F, false, false);
            this.drawScaledString((String)betsInfos.get("stats_total"), this.guiLeft + 381 - this.fontRenderer.getStringWidth((String)betsInfos.get("stats_total")), this.guiTop + 37, 16777215, 1.0F, false, false);

            if (mouseX > this.guiLeft + 131 && mouseX < this.guiLeft + 131 + 75 && mouseY > this.guiTop + 30 && mouseY < this.guiTop + 30 + 20)
            {
                tooltipToDraw = I18n.getString("enterprise.bet.tooltip.active");
            }
            else if (mouseX > this.guiLeft + 220 && mouseX < this.guiLeft + 220 + 75 && mouseY > this.guiTop + 30 && mouseY < this.guiTop + 30 + 20)
            {
                tooltipToDraw = I18n.getString("enterprise.bet.tooltip.reward");
            }
            else if (mouseX > this.guiLeft + 309 && mouseX < this.guiLeft + 309 + 75 && mouseY > this.guiTop + 30 && mouseY < this.guiTop + 30 + 20)
            {
                tooltipToDraw = I18n.getString("enterprise.bet.tooltip.total");
            }
            ...
            while (var5.hasNext())
            {
                ...
                String playersLabel = String.format("%.0f", new Object[] {(Double)betInfos.get("countPlayerBet")}) + " " + I18n.getString("enterprise.bet.players");
                this.drawScaledString("\u00a75" + playersLabel, offsetX + 249 - this.fontRenderer.getStringWidth(playersLabel), offsetY.intValue() + 14, 16777215, 1.0F, false, false);
                String diffText;

                if (((String)betInfos.get("status")).contains("bet#"))
                {
                    this.drawScaledString(I18n.getString("enterprise.bet.status.bet") + " (" + ((String)betInfos.get("status")).split("#")[1] + "$)", offsetX + 4, offsetY.intValue() + 24, 16777215, 1.0F, false, false);
                }
                else
                {
                    diffText = I18n.getString("enterprise.bet.status." + betInfos.get("status"));
                ...
                if (!betInfos.get("status").equals("done") && ((Double)betInfos.get("deadlineTime")).doubleValue() > 0.0D)
                {
                    ...
                    if (action > 0L)
                    {
                        ...
                        diffText = I18n.getString("enterprise.contract.ends_in") + diffText;
                    }
                    else
                    {
                        diffText = I18n.getString("enterprise.contract.expired");
                        expired = true;
                    }
                ...
                this.drawScaledString("\u00a78" + diffText, offsetX + 249 - this.fontRenderer.getStringWidth(diffText), offsetY.intValue() + 24, 16777215, 1.0F, false, false);
                ...
                if (this.openedBetId != "" && this.openedBetId == betInfos.get("id"))
                {
                    ...
                    if (!betInfos.get("status").equals("done") && EnterpriseGui.hasPermission("bets") && ((Boolean)EnterpriseGui.enterpriseInfos.get("isInEnterprise")).booleanValue())
                    {
                        ...
                        this.drawScaledString(I18n.getString("enterprise.bet.btn_label.end"), offsetX + 224, offsetY.intValue() + 71, 16777215, 1.0F, true, false);
                        action1 = "end";
                        ClientEventHandler.STYLE.bindTexture("enterprise_bet");
                        ...
                        this.drawScaledString(I18n.getString("enterprise.bet.btn_label.cancel"), offsetX + 168, offsetY.intValue() + 71, 16777215, 1.0F, true, false);
                        action2 = "cancel";
                    }
                    else if (betInfos.get("status").equals("bet"))
                    {
                        this.drawScaledString("\u00a76" + I18n.getString("enterprise.bet.mise") + " " + betInfos.get("playerBet") + "$", offsetX + 4, offsetY.intValue() + 73, 16777215, 1.0F, false, false);
                    }
                    else if (!expired && betInfos.get("status").equals("to_bet"))
                    {
                        ...
                        ClientEventHandler.STYLE.bindTexture("enterprise_bet");
                        ModernGui.drawModalRectWithCustomSizedTexture((float)(offsetX + 196), (float)(offsetY.intValue() + 65), 121, 270, 55, 18, 512.0F, 512.0F, false);
                        this.drawScaledString(I18n.getString("enterprise.bet.btn_label.bet"), offsetX + 224, offsetY.intValue() + 71, 16777215, 1.0F, true, false);
                        ClientEventHandler.STYLE.bindTexture("enterprise_bet");
                        ...
                        if (this.lastInputX != offsetX + 6 || this.lastInputY != offsetY.intValue() + 69)
                        {
                            ...
                            this.betInput = new GuiTextField(this.fontRenderer, offsetX + 7, offsetY.intValue() + 71, 54, 14);
                            this.betInput.setEnableBackgroundDrawing(false);
                            this.betInput.setMaxStringLength(6);
                        ...
                        this.drawScaledString(I18n.getString("enterprise.bet.minBet") + ": " + String.format("%.0f", new Object[] {(Double)betInfos.get("minBet")}) + "$", offsetX + 82, offsetY.intValue() + 71, 11842740, 1.0F, false, false);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton)
    {
        ...
        if (mouseButton == 0)
        {
            if (this.createButton.enabled && EnterpriseGui.hasPermission("bets") && mouseX >= this.guiLeft + 10 && mouseX <= this.guiLeft + 10 + 100 && mouseY >= this.guiTop + 165 && mouseY <= this.guiTop + 165 + 20)
            {
                Minecraft.getMinecraft().displayGuiScreen(new EnterpriseBetCreateGui(this));
            }

            if (!this.hoveredAction.isEmpty() && this.openedBetId != "")
            {
                if (!this.betInput.getText().isEmpty() && (!this.isNumeric(this.betInput.getText()) || (double)Integer.parseInt(this.betInput.getText()) < this.hoveredMinBet))
                {
                    return;
                }
                ...
                if (System.currentTimeMillis() - this.lastAction.longValue() > 800L)
                {
                    ...
                    PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new EnterpriseBetActionPacket((String)EnterpriseGui.enterpriseInfos.get("name"), this.openedBetId, this.hoveredAction, !this.betInput.getText().isEmpty() ? Integer.parseInt(this.betInput.getText()) : 0, this.selectedOption)));
                    this.hoveredAction = "";
                    this.openedBetId = "";
