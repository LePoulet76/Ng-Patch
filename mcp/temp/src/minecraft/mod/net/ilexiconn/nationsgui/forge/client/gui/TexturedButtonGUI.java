...
public class TexturedButtonGUI extends GuiButton {
   ...
   public TexturedButtonGUI(int id, int x, int y, int width, int height, String texture, int u, int v, String text) {
   ...
   public void func_73737_a(Minecraft mc, int mouseX, int mouseY) {
      if(this.field_73748_h) {
         ...
         this.func_73731_b(mc.field_71466_p, this.field_73744_e, this.field_73746_c + this.field_73747_a - mc.field_71466_p.func_78256_a(this.field_73744_e) - 6, this.field_73743_d + (this.field_73745_b - 8) / 2, 14737632);
      }
   ...
   public void drawContinuousTexturedBox(int x, int y, int u, int v, int width, int height, int textureWidth, int textureHeight, int topBorder, int bottomBorder, int leftBorder, int rightBorder, float zLevel) {
      ...
      for(j = 0; j < xPasses + (remainderWidth > 0?1:0); ++j) {
         this.func_73729_b(x + leftBorder + j * fillerWidth, y, u + leftBorder, v, j == xPasses?remainderWidth:fillerWidth, topBorder);
         this.func_73729_b(x + leftBorder + j * fillerWidth, y + topBorder + canvasHeight, u + leftBorder, v + topBorder + fillerHeight, j == xPasses?remainderWidth:fillerWidth, bottomBorder);

         for(int j1 = 0; j1 < yPasses + (remainderHeight > 0?1:0); ++j1) {
            this.func_73729_b(x + leftBorder + j * fillerWidth, y + topBorder + j1 * fillerHeight, u + leftBorder, v + topBorder, j == xPasses?remainderWidth:fillerWidth, j1 == yPasses?remainderHeight:fillerHeight);
         }
      }
