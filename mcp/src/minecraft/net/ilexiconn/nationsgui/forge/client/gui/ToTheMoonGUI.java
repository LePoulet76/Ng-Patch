...
public class ToTheMoonGUI extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        this.btn = new ToTheMoonGUI$ToTheMoonGuiButton(0, (this.width - 289) / 2 + 174, (this.height - 172) / 2 + 135, I18n.getString(this.isButtonActive ? "moon.contribute" : "moon.achieved"), this.isButtonActive);
        this.buttonList.add(this.btn);
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float par3)
    {
        ...
        GL11.glTranslatef((float)(-(this.guiLeft + 296 + this.helpSectionOffsetX)), (float)(-(this.guiTop + 147)), 0.0F);
        this.drawScaledString(I18n.getString("moon.label.leader"), this.guiLeft + 296 + this.helpSectionOffsetX, this.guiTop + 147, 0, 1.0F, false, false);
        GL11.glPopMatrix();
        ClientEventHandler.STYLE.bindTexture("to_the_moon");
        ...
        this.drawScaledString(I18n.getString("moon.label.leader.title"), this.guiLeft + 286 - 88 + 44 + this.helpSectionOffsetX, this.guiTop + 17, 0, 1.0F, true, false);
        int percent;
        ...
        if (this.donators != null && !this.donators.isEmpty())
        {
            for (percent = 0; percent < Math.min(this.donators.size(), 10); ++percent)
            {
                ...
                this.drawScaledString("\u00a72" + ((String)this.donators.get(percent)).split("#")[1] + "$", this.guiLeft + 294 - 88 + this.helpSectionOffsetX + this.fontRenderer.getStringWidth(progress + ". ") - 2, this.guiTop + 37 + percent * 13, 0, 0.5F, false, false);
            }
        }
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef((float)((this.width - xSize) / 2 + 17), (float)((this.height - ySize) / 2 + 145), 0.0F);
        GL11.glRotatef(-90.0F, 0.0F, 0.0F, 1.0F);
        GL11.glTranslatef((float)(-((this.width - xSize) / 2 + 17)), (float)(-((this.height - ySize) / 2 + 145)), 0.0F);
        this.drawScaledString(I18n.getString("moon.title"), (this.width - xSize) / 2 + 17, (this.height - ySize) / 2 + 145, 16777215, 1.2F, false, false);
        GL11.glPopMatrix();
        ...
        this.drawScaledString(percent + "%", (this.width - xSize) / 2 + 147, (this.height - ySize) / 2 + 141, 16777215, 0.8F, true, false);
        this.drawCenteredString(this.fontRenderer, Math.round(this.goal) + "$", (this.width - xSize) / 2 + 94, (this.height - ySize) / 2 + 22, 16777215);
        ClientEventHandler.STYLE.bindTexture("to_the_moon");
        ModernGui.drawModalRectWithCustomSizedTexture((float)((this.width - xSize) / 2 + 139), (float)((this.height - ySize) / 2 + 22 + (117 - percent)), 0, 212 + (117 - percent), 13, 117 - percent, 512.0F, 512.0F, false);
        String[] descriptionWords = I18n.getString("moon.desc").split(" ");
        String line = "";
        int lineNumber = 0;
        ...
        for (int var16 = 0; var16 < var15; ++var16)
        {
            ...
            if (this.fontRenderer.getStringWidth(line + descWord) <= 100)
            {
                if (!line.equals(""))
                {
                    line = line + " ";
            ...
            {
                this.drawScaledString(line, (this.width - xSize) / 2 + 170, (this.height - ySize) / 2 + 28 + lineNumber * 10, 16777215, 0.8F, false, true);
                ++lineNumber;
                line = descWord;
            }
        }
        ...
        if (mouseX >= (this.width - xSize) / 2 + 233 && mouseX <= (this.width - xSize) / 2 + 243 && mouseY >= (this.height - ySize) / 2 + 115 && mouseY <= (this.height - ySize) / 2 + 125)
        {
            this.drawHoveringText(Arrays.asList(I18n.getString("moon.info").split("##")), mouseX, mouseY, this.fontRenderer);
        }
    }
    ...
    protected void actionPerformed(GuiButton btn)
    {
        switch (btn.id)
        {
            case 0:
                ...
                {
                    Minecraft.getMinecraft().displayGuiScreen(new ToTheMoonGUI$ToTheMoonModalGui(this));
                }
