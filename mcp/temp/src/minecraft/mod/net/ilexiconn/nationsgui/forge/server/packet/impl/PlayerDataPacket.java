...
public class PlayerDataPacket implements IServerPacket, IClientPacket, IPacket {
   ...
   public void fromBytes(ByteArrayDataInput data) {
      try {
         this.emotes = NGPlayerData.nbtToList(Packet.func_73283_d(data));
         this.currentEmotes = NGPlayerData.nbtToList(Packet.func_73283_d(data));
         this.capes = NGPlayerData.nbtToList(Packet.func_73283_d(data));
   ...
   public void toBytes(ByteArrayDataOutput data) {
      try {
         byte[] e = CompressedStreamTools.func_74798_a(NGPlayerData.listToNBT(this.emotes));
         data.writeShort(e.length);
         data.write(e);
         byte[] currentEmotesData = CompressedStreamTools.func_74798_a(NGPlayerData.listToNBT(this.currentEmotes));
         data.writeShort(currentEmotesData.length);
         data.write(currentEmotesData);
         byte[] capesData = CompressedStreamTools.func_74798_a(NGPlayerData.listToNBT(this.capes));
