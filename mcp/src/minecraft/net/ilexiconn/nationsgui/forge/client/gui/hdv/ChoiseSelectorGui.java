...
public class ChoiseSelectorGui<T extends Object> extends Gui
{
    ...
    public void draw(int mouseX, int mouseY)
    {
        ...
        drawRect(this.posX + 14, this.posY, this.posX + this.width - 1, this.posY + 16, -13487566);
        drawRect(this.posX + this.width - 1, this.posY + 1, this.posX + this.width, this.posY + 15, -13487566);
        FontRenderer fontRenderer = Minecraft.getMinecraft().fontRenderer;
        String text = (String)this.currentSelection.getKey();
        fontRenderer.drawString(text, this.posX + 14 + (this.width - 14) / 2 - fontRenderer.getStringWidth(text) / 2, this.posY + 5, 16777215);
        ...
        if (this.open)
        {
            ...
            for (Iterator var6 = this.stringComparatorMap.entrySet().iterator(); var6.hasNext(); ++i)
            {
                ...
                drawRect(this.posX, y, this.posX + this.width, y + 14, color);
                fontRenderer.drawString((String)entry.getKey(), this.posX + this.width / 2 - fontRenderer.getStringWidth((String)entry.getKey()) / 2, y + 2, 16777215);
            }
        }
    }
        ...
        else if (this.open)
        {
            ...
            for (Iterator var4 = this.stringComparatorMap.entrySet().iterator(); var4.hasNext(); ++i)
            {
                ...
                if (mouseX >= this.posX && mouseX <= this.posX + this.width && mouseY >= y && mouseY <= y + 14)
                {
                    ...
                    PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new MarketPacket(MarketGui.search.getText(), (String)entry.getKey(), MarketGui.currentPage - 1, MarketGui.mySalesOnly)));
                    this.open = false;
                    return true;
                }
            }
