...
public class EnterpriseFlagGui
extends GuiScreen {
    ...
    public EnterpriseFlagGui(String targetName) {
        ...
        if (EnterpriseGui.enterpriseInfos.get("flagPixels") != null && ((ArrayList)EnterpriseGui.enterpriseInfos.get("flagPixels")).size() > 0) {
            for (int i = 0; i < ((ArrayList)EnterpriseGui.enterpriseInfos.get("flagPixels")).size(); ++i) {
                this.pixels.put(i, ((Double)((ArrayList)EnterpriseGui.enterpriseInfos.get("flagPixels")).get(i)).intValue());
            }
        } else {
            for (int i = 0; i < 625; ++i) {
                this.pixels.put(i, -1);
    ...
    public void func_73866_w_() {
        ...
        this.saveButton = new GuiButton(0, this.guiLeft + 244, this.guiTop + 151, 54, 20, I18n.func_135053_a((String)"faction.flag.valid"));
    }
    ...
    public void func_73863_a(int mouseX, int mouseY, float par3) {
        ...
        for (i = 0; i < this.colorList.size(); ++i) {
            ...
            if (colodId == this.colorSelected) {
                ClientEventHandler.STYLE.bindTexture("enterprise_flag");
                ModernGui.drawModalRectWithCustomSizedTexture(this.guiLeft + 244 + x * 18 - 3, this.guiTop + 22 + y * 18 - 3, 22, 182, 24, 24, 512.0f, 512.0f, false);
            }
            x = x < 2 ? x + 1 : 0;
            y = x == 0 ? y + 1 : y;
        }
        ...
        if (this.pixels.size() > 0) {
            for (Map.Entry<Integer, Integer> pair : this.pixels.entrySet()) {
                ...
                Gui.func_73734_a((int)(this.guiLeft + 58 + x * 6), (int)(this.guiTop + 15 + y * 6), (int)(this.guiLeft + 58 + x * 6 + 6), (int)(this.guiTop + 15 + y * 6 + 6), (int)colorId);
                x = x < 24 ? x + 1 : 0;
                y = x == 0 ? y + 1 : y;
                ++i;
            }
        }
        this.drawScaledString(I18n.func_135053_a((String)"enterprise.flag.reset"), this.guiLeft + 261, this.guiTop + 137, 0x191919, 1.0f, false, false);
    ...
    protected void func_73864_a(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            ...
            if (EnterpriseGui.hasPermission("flag") && mouseX > this.guiLeft + 244 && mouseX < this.guiLeft + 244 + 54 && mouseY > this.guiTop + 151 && mouseY < this.guiTop + 151 + 18) {
                ...
                graphics2D.dispose();
                String base64Img = EnterpriseFlagGui.encodeToString(image, "png").replace("\r\n", "");
                ...
                dataToPacket.put("mainFlagColor", EnterpriseFlagGui.mostCommon(imagePixels));
                this.field_73882_e.field_71416_A.func_77366_a("random.successful_hit", 1.0f, 1.0f);
                PacketDispatcher.sendPacketToServer((Packet)PacketRegistry.INSTANCE.generatePacket(new EnterpriseSaveFlagDataPacket(dataToPacket)));
                Minecraft.func_71410_x().func_71373_a((GuiScreen)new EnterpriseSettingsGUI());
            }
        }
