            ...
            if (!GenericOverride.displaysCountryEntry())
            {
                if (!NotificationOverride.displaysNotification())
                {
                    if (!ClientData.currentWarzone.isEmpty())
                    {
                        ...
                        ModernGui.drawScaledCustomSizeModalRect((float)(resolution.getScaledWidth() / 2) - 148.0F, 0.0F, 1180.0F, 880.0F, 740, 153, 296, 61, 1920.0F, 1033.0F, true);
                        String warzoneName = "Warzone " + (String)ClientData.currentWarzone.get("name");
                        ModernGui.drawScaledStringCustomFont(warzoneName, (float)(resolution.getScaledWidth() / 2), 15.0F, 16000586, 0.5F, "center", true, "minecraftDungeons", 18);
                        ModernGui.drawScaledStringCustomFont(!((String)ClientData.currentWarzone.get("factionId")).isEmpty() ? ((String)ClientData.currentWarzone.get("factionName")).replaceAll("Empire", "Emp") : I18n.getString("overlay.warzone.free"), (float)(resolution.getScaledWidth() / 2), 22.0F, 16777215, 0.5F, "center", true, "minecraftDungeons", 25);
                        float progress = Float.parseFloat((String)ClientData.currentWarzone.get("percent")) / 100.0F;
                        ModernGui.bindTextureOverlayMain();
                        ModernGui.drawScaledCustomSizeModalRect((float)(resolution.getScaledWidth() / 2) - 83.6F, 42.0F, 1502.0F, 807.0F, 418, 11, 167, 4, 1920.0F, 1033.0F, true);
                        ModernGui.drawScaledCustomSizeModalRect((float)(resolution.getScaledWidth() / 2) - 83.6F, 42.0F, 1502.0F, 831.0F, (int)(418.0F * progress), 11, (int)((float)((int)(418.0F * progress)) / 2.5F), 4, 1920.0F, 1033.0F, true);
                        ModernGui.drawScaledStringCustomFont((String)ClientData.currentWarzone.get("percent") + "%", (float)(resolution.getScaledWidth() / 2), 47.0F, 16777215, 0.5F, "center", true, "minecraftDungeons", 25);
                    }
                }
            }
        }
