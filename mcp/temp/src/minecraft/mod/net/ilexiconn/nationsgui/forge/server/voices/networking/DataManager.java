...
public class DataManager implements ITickHandler {
   ...
   public void sendVoiceToWorldWithinDistance(ServerDatalet let, int distance) {
      ...
      if(let.end) {
         for(i = 0; i < players.size(); ++i) {
            ...
            if(player != curPlayer) {
               d4 = curPlayer.field_70165_t - player.field_70165_t;
               d5 = curPlayer.field_70163_u - player.field_70163_u;
               d6 = curPlayer.field_70161_v - player.field_70161_v;
   ...
   public void playerDisconnect(EntityPlayer player) {
      List list = (List)this.receivedEntityData.get(Integer.valueOf(player.field_70157_k));
      if(list != null) {
         this.receivedEntityData.remove(Integer.valueOf(player.field_70157_k));
      }
      ...
      while(it.hasNext()) {
         ...
         for(int i = 0; i < idList.size(); ++i) {
            if(((Integer)idList.get(i)).intValue() == player.field_70157_k) {
               idList.remove(i);
            }
         }
      }
