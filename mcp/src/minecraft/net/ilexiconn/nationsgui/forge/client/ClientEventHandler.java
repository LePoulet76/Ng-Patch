...
public class ClientEventHandler
{
    ...
    public PlayerListGUI playerListGUI = new PlayerListGUI(Minecraft.getMinecraft());
    ...
    ClientEventHandler()
    {
        ...
        this.fovMethod = ReflectionHelper.findMethod(EntityRenderer.class, Minecraft.getMinecraft().entityRenderer, new String[] {"getFOVModifier", "getFOVModifier", "a"}, new Class[] {Float.TYPE, Boolean.TYPE});
    }

    public static TextureObject setResourceUrl(ResourceLocation loc, String url)
    {
        TextureManager texturemanager = Minecraft.getMinecraft().renderEngine;
        ThreadDownloadImageData threadDownloadImageData = (ThreadDownloadImageData)texturemanager.getTexture(loc);
        ...
        if (threadDownloadImageData == null)
        {
            ...
            texturemanager.loadTexture(loc, threadDownloadImageData);
        }
    ...
    public static boolean isPlayerTalk(String player)
    {
        try
        {
            ...
            while (var1.hasNext())
            {
                ...
                if (stream != null && stream.isEntityPlayer() && stream.getPlayer() != null && stream.getPlayer().username.equals(player))
                {
                    return true;
                }
            }
    ...
    public void preRender(Pre event)
    {
        ...
        List capeSkins = ClientProxy.SKIN_MANAGER.getPlayerActiveSkins(event.entityPlayer.username, SkinType.CAPES);
        ...
        {
            location = AbstractClientPlayer.getLocationCape(player.username);
            ObfuscationReflectionHelper.setPrivateValue(AbstractClientPlayer.class, player, location, 4);
            ObfuscationReflectionHelper.setPrivateValue(AbstractClientPlayer.class, player, setResourceUrl(location, AbstractClientPlayer.getCapeUrl(player.username)), 2);
        }
    }
        ...
        else if (ClientData.textureClean)
        {
            if (event.gui instanceof GuiMainMenu && !(event.gui instanceof MainGUI))
            {
                ...
                if (ClientProxy.clientConfig != null)
                {
                    ...
                    if (ClientProxy.clientConfig.FirstSetupClient)
                    {
                        ...
                        {
                            Minecraft.getMinecraft().gameSettings.keyBindForward.keyCode = 44;
                            Minecraft.getMinecraft().gameSettings.keyBindBack.keyCode = 31;
                            Minecraft.getMinecraft().gameSettings.keyBindLeft.keyCode = 16;
                            Minecraft.getMinecraft().gameSettings.keyBindRight.keyCode = 32;
                            Minecraft.getMinecraft().gameSettings.keyBindDrop.keyCode = 30;
                            Minecraft.getMinecraft().gameSettings.language = "fr_FR";
                        }
                    }
                }
            }
            ...
            else
            {
                ...
                if (event.gui instanceof GuiOptions && !(event.gui instanceof OptionsGUI))
                {
                    try
                    {
                        e = GuiOptions.class.getDeclaredField(NationsGUITransformer.inDevelopment ? "parentScreen" : "parentScreen");
                ...
                else if (event.gui instanceof GuiGameOver && !(Minecraft.getMinecraft().currentScreen instanceof CustomGuiGameOver) && ClientProxy.multiRespawn)
                {
                ...
                {
                    event.gui = new InventoryGUI(Minecraft.getMinecraft().thePlayer);
                }
                else if (event.gui instanceof ScreenChatOptions && !(event.gui instanceof MultiplayerConfigGUI))
                {
                    try
                    {
                        e = ScreenChatOptions.class.getDeclaredField(NationsGUITransformer.inDevelopment ? "theGuiScreen" : "theGuiScreen");
                        e.setAccessible(true);
                        event.gui = new MultiplayerConfigGUI((GuiScreen)e.get(event.gui), Minecraft.getMinecraft().gameSettings);
                    }
            ...
            {
                Minecraft.getMinecraft().gameSettings.guiScale = this.scaleGUIBase;
                this.scaleGUIBase = -1;
            }
            else if (event.gui != null && event.gui instanceof AbstractFirstConnectionGui)
            {
                this.scaleGUIBase = Minecraft.getMinecraft().gameSettings.guiScale;
    ...
    {
        event.manager.addSound("nationsgui:buy.ogg");
        event.manager.addSound("nationsgui:alert.ogg");
    }
    ...
    public void onJoinWorld(EntityJoinWorldEvent event)
    {
        ...
        if (NationsGUI.RELEASE_TYPE == ReleaseType.DEVELOP && event.entity == Minecraft.getMinecraft().thePlayer)
        {
            Minecraft.getMinecraft().thePlayer.sendChatToPlayer((new ChatMessageComponent()).setColor(EnumChatFormatting.RED).setBold(Boolean.valueOf(true)).addText("You\'re running a development build of NationsGUI"));
        }
    }
    ...
    public void onRenderPlayerPost(Chat event)
    {
        ...
        int mouseYChat = event.resolution.getScaledHeight() - event.mouseY - 29;
    ...
    {
        if (Minecraft.isGuiEnabled() && !event.entityPlayer.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer))
        {
            String username = event.entityPlayer.username;
            List buddySkins = ClientProxy.SKIN_MANAGER.getPlayerActiveSkins(event.entityPlayer.username, SkinType.BUDDIES);
            BuddySkin buddySkin = buddySkins.size() > 0 ? (BuddySkin)buddySkins.get(0) : null;

            if (buddySkin != null)
            {
                float bob = MathHelper.sin(((float)event.entity.ticksExisted + event.partialRenderTick) / 15.0F) * 0.1F;
                GL11.glPushMatrix();
                GL11.glDisable(GL11.GL_CULL_FACE);
                GL11.glEnable(GL12.GL_RESCALE_NORMAL);
                GL11.glRotatef(-GUIUtils.interpolate(event.entityPlayer.prevRenderYawOffset, event.entityPlayer.renderYawOffset, event.partialRenderTick), 0.0F, 1.0F, 0.0F);
                GL11.glTranslatef(0.0F, -0.3F + bob, 0.0F);
                GL11.glScalef(-0.5F, -0.5F, 0.5F);
                GL11.glRotatef(0.0F, 0.0F, 1.0F, 0.0F);
                GL11.glRotatef(-GUIUtils.interpolate(event.entityPlayer.prevRotationYawHead, event.entityPlayer.rotationYawHead, event.partialRenderTick), 0.0F, 1.0F, 0.0F);
                ...
                {
                    resourceLocation = AbstractClientPlayer.getLocationSkull(aliasName);
                    AbstractClientPlayer.getDownloadImageSkin(resourceLocation, aliasName);
                }

                Minecraft.getMinecraft().renderEngine.bindTexture(resourceLocation);
                GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
                SkullItemRenderer.MODEL_SKULL_LARGE.render((Entity)null, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0625F);
                GL11.glPopMatrix();
            }
        }
    }
    ...
    {
        if (!event.entity.canEntityBeSeen(Minecraft.getMinecraft().renderViewEntity) && (event.entity instanceof EntityAnimal || event.entity instanceof EntityMob) && !(event.entity instanceof EntityHorse))
        {
            event.setCanceled(true);
        }

        if (event.entity instanceof EntityPlayer && !event.entity.isSneaking() && event.entity.canEntityBeSeen(Minecraft.getMinecraft().renderViewEntity))
        {
            event.setCanceled(true);
        }
    }
    ...
    {
        String username = Minecraft.getMinecraft().thePlayer.username;
    ...
    public void onRenderWorldLast(RenderWorldLastEvent event)
    {
        ...
        if (Minecraft.isGuiEnabled())
        {
            ...
            Set entities = (Set)ReflectionHelper.getPrivateValue(WorldClient.class, client, new String[] {"entityList", "entityList", "J"});
            ...
            while (var14.hasNext())
            {
                ...
                if (entity != null && entity instanceof EntityLivingBase && entity.isInRangeToRenderVec3D((new Vector3(renderingVector.xCoord, renderingVector.yCoord, renderingVector.zCoord)).toVec3()) && (entity.ignoreFrustumCheck || frustrum.isBoundingBoxInFrustum(entity.boundingBox)) && entity.isEntityAlive() && !entity.isSneaking())
                {
    ...
    private void renderMarkers()
    {
        ...
        while (var2.hasNext())
        {
            ...
            double dx = markerX - player.posX;
            double dy = markerY - player.posY;
            double dz = markerZ - player.posZ;
            ...
            GL11.glPushMatrix();
            GL11.glTranslatef((float)(markerX - RenderManager.renderPosX), (float)(markerY - RenderManager.renderPosY + 0.0D + 0.6000000238418579D), (float)(markerZ - RenderManager.renderPosZ));
            ...
            else if (markerData.get("text") != null && ((String)markerData.get("name")).contains("autel#"))
            {
                ...
                ModernGui.drawScaledStringCustomFont("\u00a7c" + I18n.getString("marker.autel") + " " + ((String)markerData.get("name")).replace("autel#", ""), 22.0F, -3.0F, 7988722, 0.5F, "center", true, "minecraftDungeons", 20);
            }
    ...
    {
        if (entity2.riddenByEntity == null)
        {
            EntityLivingBase entity = entity2;

            if (entity2.ridingEntity != null && entity2.ridingEntity instanceof EntityLivingBase)
            {
                entity = (EntityLivingBase)entity2.ridingEntity;
            }
            ...
            if (entity != null)
            {
                if (entity2.getDistanceSqToEntity(viewPoint) <= (double)(entity2 instanceof EntityPlayer ? 4096 : 256) && entity2 != mc.thePlayer && (!entity.isInvisible() || entity2 instanceof EntityPlayer && isPlayerTalk(((EntityPlayer)entity2).username)) && entity.canEntityBeSeen(viewPoint))
                {
                    double riddenBy = entity2.lastTickPosX + (entity2.posX - entity2.lastTickPosX) * (double)partialTicks;
                    double y = entity2.lastTickPosY + (entity2.posY - entity2.lastTickPosY) * (double)partialTicks;
                    double z = entity2.lastTickPosZ + (entity2.posZ - entity2.lastTickPosZ) * (double)partialTicks;
                    ...
                    {
                        GL11.glPushMatrix();
                        GL11.glTranslatef((float)(riddenBy - RenderManager.renderPosX), (float)(y - RenderManager.renderPosY + (double)entity2.height + 0.6000000238418579D), (float)(z - RenderManager.renderPosZ));
                        GL11.glNormal3f(0.0F, 1.0F, 0.0F);
                        GL11.glRotatef(-RenderManager.instance.playerViewY, 0.0F, 1.0F, 0.0F);
                        GL11.glRotatef(RenderManager.instance.playerViewX, 1.0F, 0.0F, 0.0F);
                        GL11.glScalef(-scale, -scale, scale);
                        ...
                        {
                            String padding = "";
                            ScorePlayerTeam minBoxWidth = ((EntityPlayer)entity2).getWorldScoreboard().getPlayersTeam(((EntityPlayer)entity2).getCommandSenderName());

                            if (minBoxWidth != null && (minBoxWidth.getColorPrefix().contains("ATTAQUANT") || minBoxWidth.getColorPrefix().contains("DEFENSEUR") || minBoxWidth.getColorPrefix().matches("\u00a7.{1}")))
                            {
                                padding = minBoxWidth.getColorPrefix();
                            }

                            if (ClientProxy.serverType.equals("build") && playersIslandTeamPrefix.containsKey(((EntityPlayer)entity2).username))
                            {
                                padding = (String)playersIslandTeamPrefix.get(((EntityPlayer)entity2).username);
                            }

                            if (ClientProxy.currentServerName.equals("aff") && minBoxWidth != null)
                            {
                                padding = minBoxWidth.getColorPrefix() + "[" + minBoxWidth.func_96669_c().substring(0, 1).toUpperCase() + "] ";
                            }

                            if (!ClientData.currentFoot.isEmpty())
                            {
                                if (ClientData.currentFoot.containsKey("team1Players") && !((String)ClientData.currentFoot.get("team1Players")).isEmpty() && ((String)ClientData.currentFoot.get("team1Players")).toLowerCase().contains(((EntityPlayer)entity2).username.toLowerCase()))
                                {
                                    padding = "\u00a7" + (String)ClientData.currentFoot.get("team1Color") + "[" + (String)ClientData.currentFoot.get("team1") + "] ";
                                }

                                if (ClientData.currentFoot.containsKey("team2Players") && !((String)ClientData.currentFoot.get("team2Players")).isEmpty() && ((String)ClientData.currentFoot.get("team2Players")).toLowerCase().contains(((EntityPlayer)entity2).username.toLowerCase()))
                                {
                            ...
                            if (ClientProxy.serverType.equals("ng") && ClientData.eventsInfos.size() > 0)
                            {
                                ...
                                if (sizeY != null)
                                {
                                    ...
                                    while (maxHpStr.hasNext())
                                    {
                                        ...
                                        if (text.contains(((EntityPlayer)entity2).username))
                                        {
                                ...
                                if (!height && ((List)sizeX.get("players")).contains(((EntityPlayer)entity2).username))
                                {
                                    padding = "\u00a77[" + (!sizeX.get("displayName").equals("") ? sizeX.get("displayName") : sizeX.get("name")) + "] \u00a7r";
                                }
                            }

                            name = padding + ((EntityPlayer)entity2).getCommandSenderName();
                        }
                        ...
                        if (entity2 instanceof EntityPlayer)
                        {
                            ...
                            {
                                if (!playersFaction.containsKey(var35.username))
                                {
                                    PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new TagPlayerFactionPacket(var35.username)));
                                    playersFaction.put(var35.username, "");
                                }
                                else if (!((String)playersFaction.get(var35.username)).equals("") && !((String)playersFaction.get(var35.username)).equals("no_country") && !((String)playersFaction.get(var35.username)).equals("\u00a77Sans pays") && !factionsFlag.containsKey(((String)playersFaction.get(var35.username)).replaceFirst("\u00a7.{1}", "")))
                                {
                                    PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new TagFactionFlagPacket((String)playersFaction.get(var35.username))));
                                }
                            }

                            if (ClientProxy.serverType.equals("build") && (!playersIslandTeamPrefix.containsKey(var35.username) || playersIslandTeamPrefixLastRefresh.containsKey(var35.username) && System.currentTimeMillis() - ((Long)playersIslandTeamPrefixLastRefresh.get(var35.username)).longValue() > 10000L))
                            {
                                playersIslandTeamPrefixLastRefresh.put(var35.username, Long.valueOf(System.currentTimeMillis()));
                                PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new IslandGetTeamPrefixPacket(var35.username)));

                                if (!playersIslandTeamPrefix.containsKey(var35.username))
                                {
                                    playersIslandTeamPrefix.put(var35.username, "");
                                }
                            }
                        }
                        ...
                        if (entity instanceof EntityPlayer)
                        {
                            ...
                            if (GetGroupAndPrimePacket.NGPRIME_PLAYERS.contains(p.username))
                            {
                                ...
                                {
                                    Minecraft.getMinecraft().getTextureManager().bindTexture((ResourceLocation)NationsGUI.BADGES_RESOURCES.get("badges_ngprime"));
                                    ModernGui.drawModalRectWithCustomSizedTexture(-((float)Minecraft.getMinecraft().fontRenderer.getStringWidth("NGPrime") * 0.6F / 2.0F) - 0.0F - 3.0F - 2.0F, -28.0F, 0, 0, 6, 6, 6.0F, 6.0F, false);
                            ...
                            if (playersFaction.containsKey(p.username))
                            {
                                var43 = (String)playersFaction.get(p.username);

                                if (var43.equals("no_country"))
                                {
                                    var43 = I18n.getString("overlay.no_country");
                                }

                                if ((float)mc.fontRenderer.getStringWidth(var43.replaceFirst("\u00a7.{1}", "")) * 0.5F + 12.0F + 2.0F > (float)var33)
                                {
                                    var33 = (int)((float)mc.fontRenderer.getStringWidth(var43.replaceFirst("\u00a7.{1}", "")) * 0.5F + 12.0F + 2.0F + (float)(var36 * 2));
                                    var34 = var33 / 2;
                                }
                            }

                            if ((float)mc.fontRenderer.getStringWidth(name) * 0.75F + 6.0F + 2.0F > (float)var33)
                            {
                                var33 = (int)((float)mc.fontRenderer.getStringWidth(name) * 0.75F + 6.0F + 2.0F + (float)(var36 * 2));
                                var34 = var33 / 2;
                            }

                            this.drawRect(var34, -var38 - 8, -var34, var38 - 5, this.getBadgeBGColor((String)GetGroupAndPrimePacket.GRP_BADGES_PLAYERS.get(p.username)));

                            if (ClientProxy.serverType.equals("ng"))
                            {
                                var43 = ClientData.topPlayersSkills.containsKey(p.username) ? (String)ClientData.topPlayersSkills.get(p.username) : "";

                                if (!var43.isEmpty())
                                {
                                    STYLE.bindTexture("overlay_" + var43);
                                    ModernGui.drawScaledCustomSizeModalRect((float)(-var34 - 2), GetGroupAndPrimePacket.NGPRIME_PLAYERS.contains(p.username) ? -38.0F : -28.0F, 0.0F, 0.0F, 866, 141, var34 * 2 + 4, 10, 866.0F, 141.0F, false);
                                }
                            }

                            if (NationsGUI.BADGES_RESOURCES.containsKey(GetGroupAndPrimePacket.GRP_BADGES_PLAYERS.get(p.username)))
                            {
                                Minecraft.getMinecraft().getTextureManager().bindTexture((ResourceLocation)NationsGUI.BADGES_RESOURCES.get(GetGroupAndPrimePacket.GRP_BADGES_PLAYERS.get(p.username)));
                                ModernGui.drawModalRectWithCustomSizedTexture((float)(-var34 + var36), -18.0F, 0, 0, 6, 6, 6.0F, 6.0F, false);
                                ModernGui.drawScaledString(this.getBadgeTextColor((String)GetGroupAndPrimePacket.GRP_BADGES_PLAYERS.get(p.username)) + name, -var34 + var36 + 6 + 2, -18, 16777215, 0.75F, false, false);
                            }
                            else
                            {
                                ModernGui.drawScaledString(this.getBadgeTextColor((String)GetGroupAndPrimePacket.GRP_BADGES_PLAYERS.get(p.username)) + name, -var34 + var36, -18, 16777215, 0.75F, false, false);
                            }

                            if (ClientProxy.serverType.equals("ng"))
                            {
                                if (ClientProxy.playersInAdminMode.containsKey(p.username) && ((Boolean)ClientProxy.playersInAdminMode.get(p.username)).booleanValue() && Minecraft.isGuiEnabled())
                                {
                                    Minecraft.getMinecraft().getTextureManager().bindTexture(AssistancePlayerGUI.GUI_TEXTURE);
                                    GUIUtils.drawScaledCustomSizeModalRect(-var34 + var36, -9, 1.0F, 320.0F, 37, 38, 6, 6, 512.0F, 512.0F);
                                    ModernGui.drawScaledString(I18n.getString("overlay.service"), -var34 + var36 + 6 + 2, -8, 16777215, 0.5F, false, false);
                                }
                                else if (playersFaction.containsKey(p.username) && !((String)playersFaction.get(p.username)).equals(""))
                                {
                                    var43 = (String)playersFaction.get(p.username);
                                    ...
                                    if (var43 != null && factionsFlag.containsKey(healthStr) && factionsFlag.get(healthStr) != null && (!((String)factionsFlag.get(healthStr)).equals("") || !((String)factionsFlag.get(healthStr)).contains(" ")))
                                    {
                                        ...
                                        if (factionsFlagTexture.containsKey(healthStr))
                                        {
                                            GL11.glBindTexture(GL11.GL_TEXTURE_2D, ((DynamicTexture)factionsFlagTexture.get(healthStr)).getGlTextureId());
                            ...
                            {
                                Matcher var44 = Pattern.compile(p.username + "#(\\d+)").matcher((String)((HashMap)ClientData.eventsInfos.get(0)).get("playersKills"));
                        ...
                        else
                        {
                            ...
                            if (entity instanceof IAnimalFarm)
                            {
                                ...
                                ModernGui.drawScaledString(healthStr, var34 - var36 - 6 - (int)((double)Minecraft.getMinecraft().fontRenderer.getStringWidth(healthStr) * 0.5D), -18, 16777215, 0.5F, false, false);
                                GL11.glPushMatrix();
                                GL11.glScalef(0.7F, 0.7F, 0.7F);
                                STYLE.bindTexture("hud");
                                ModernGui.drawModalRectWithCustomSizedTexture((float)(var34 - var36 - 5) / 0.7F, -25.714287F, 1, (double)(entity.getHealth() / entity.getMaxHealth()) >= 0.75D ? 58 : ((double)(entity.getHealth() / entity.getMaxHealth()) >= 0.3D ? 65 : 72), 7, 7, 256.0F, 256.0F, false);
                                GL11.glPopMatrix();
                        ...
                        if (entity2 instanceof EntityPlayer)
                        {
                            ...
                            if (VoiceChat.getProxyInstance().getSettings().isPlayerMuted(p.username))
                            {
                                STYLE.bindTexture("voice_off");
                                ModernGui.drawModalRectWithCustomSizedTexture(0.0F, 0.0F, 0, 0, 32, 32, 32.0F, 32.0F, false);
                            }
                            else if (isPlayerTalk(p.username))
                            {
                                STYLE.bindTexture("voice_vol");

                                switch ((int)(Minecraft.getSystemTime() % 1000L / 250L))
                                {
                                    case 0:
                ...
                Entity var32 = entity.riddenByEntity;
    ...
    public void onOverlayRender(net.minecraftforge.client.event.RenderGameOverlayEvent.Pre event)
    {
        ...
        while (fakeEvent.hasNext())
        {
            ...
            if (e.getType() == event.type)
            {
                ...
                if (!(Minecraft.getMinecraft().currentScreen instanceof AbstractFirstConnectionGui))
                {
                    e.renderOverride(Minecraft.getMinecraft(), event.resolution, event.partialTicks);
                }
            }
        ...
        if (Minecraft.getMinecraft().currentScreen == null && this.snackbarGUI != null && event.type == ElementType.HOTBAR)
        {
    ...
    public void onOverlayRenderPost(net.minecraftforge.client.event.RenderGameOverlayEvent.Post event)
    {
        ...
        if (ClientProxy.clientConfig.blockInfoEnabled)
        {
            ...
            if (movingObjectPosition != null && movingObjectPosition.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                int id = Minecraft.getMinecraft().theWorld.getBlockId(movingObjectPosition.blockX, movingObjectPosition.blockY, movingObjectPosition.blockZ);
                int meta = Minecraft.getMinecraft().theWorld.getBlockMetadata(movingObjectPosition.blockX, movingObjectPosition.blockY, movingObjectPosition.blockZ);
                ItemStack itemStack = new ItemStack(id, 1, meta);
                UniqueIdentifier uniqueIdentifier = GameRegistry.findUniqueIdentifierFor(Block.blocksList[id]);
                ...
                String var27 = Block.blocksList[id].getLocalizedName() + " (#" + id + ":" + meta + ")";
                int w = 20 + fontRenderer.getStringWidth(var27);
                int wMin = fontRenderer.getStringWidth(modName) + 20;
                ...
                byte h = 17;
                int durability = -1;
                TileEntity tileEntity = Minecraft.getMinecraft().theWorld.getBlockTileEntity(movingObjectPosition.blockX, movingObjectPosition.blockY, movingObjectPosition.blockZ);
                ...
                int x = resolution.getScaledWidth() / 2 - w / 2;
                byte y = 5;
                GL11.glPushMatrix();
                int l1 = -267386864;
                ...
                try
                {
                    var26.renderItemIntoGUI(fontRenderer, Minecraft.getMinecraft().getTextureManager(), itemStack, x, y);
                }
                ...
                GL11.glDisable(GL11.GL_BLEND);
                fontRenderer.drawString(var27, x + 20, y + 4, 16777215);

                if (durability != -1)
                {
                    Minecraft.getMinecraft().getTextureManager().bindTexture(DURA_ICON);
                    ModernGui.drawModalRectWithCustomSizedTexture((float)x + (float)w / 2.0F - (float)fontRenderer.getStringWidth("||||||||||||||||||||||||||||||||||||||||") / 2.0F - 12.0F, (float)(y + 17), 0, 0, 16, 16, 16.0F, 16.0F, false);
                    String color = "";
                    ...
                    fontRenderer.drawString(bar, x + w / 2 - fontRenderer.getStringWidth("||||||||||||||||||||||||||||||||||||||||") / 2 + 8, y + 20, 13703963);
                }
    ...
    {
        if (Minecraft.getMinecraft().currentScreen instanceof RecipeListGUI)
        {
            event.toolTip.add("ID " + event.itemStack.itemID + ":" + event.itemStack.getItemDamage());
        }
        else if (Minecraft.getMinecraft().currentScreen instanceof InventoryGUI && ClientProxy.serverType.equals("ng") && ClientKeyHandler.KEY_SELL.keyCode > 0)
        {
            event.toolTip.add(I18n.getStringParams("sell.press", new Object[] {Keyboard.getKeyName(ClientKeyHandler.KEY_SELL.keyCode)}));
        }
    }
    ...
    private void renderMobSpawnOverlay(Entity entity)
    {
        ...
        int y1 = (int)net.ilexiconn.nationsgui.forge.client.util.MathHelper.clip(entity.posY, 16.0D, (double)(world.getHeight() - 16));
    ...
    public int getHealthColor(Entity entity, float health, float maxHealth)
    {
        ...
        else
        {
            ...
            return ratio >= 0.75F ? (Minecraft.getMinecraft().theWorld.provider instanceof GCEdoraWorldProvider ? 2138695154 : 2130771712) : (ratio >= 0.5F ? 2146954241 : (ratio >= 0.25F ? 2146929409 : 2146898945));
        }
    }

    public void openMoonGui(boolean goalAchieved, double goal, double actualMoney, List<String> donators)
    {
        Minecraft.getMinecraft().displayGuiScreen(new ToTheMoonGUI(!goalAchieved, goal, actualMoney, donators));
    }
    ...
    public void onPlayerPreRender(Pre event)
    {
        ...
        {
            this.armorSave = (ItemStack[])event.entityPlayer.inventory.armorInventory.clone();
            ItemStack currentItem = event.entityPlayer.inventory.getCurrentItem();
            this.itemSave = currentItem;

            for (int sword = 0; sword < event.entityPlayer.inventory.armorInventory.length; ++sword)
            {
                ItemStack slot = event.entityPlayer.inventory.armorInventory[sword];

                if (slot != null && (renderLightClassBlacklist.contains(slot.getItem().getClass()) || renderLightIdsBlacklist.contains(Integer.valueOf(slot.itemID))))
                {
                    ...
                    {
                        case 0:
                            newItem = new ItemStack(Item.bootsGold);
                            break;

                        case 1:
                            newItem = new ItemStack(Item.legsGold);
                            break;

                        case 2:
                            newItem = new ItemStack(Item.plateGold);
                            break;

                        case 3:
                            newItem = new ItemStack(Item.helmetGold);
                    }

                    if (newItem != null)
                    {
                        newItem.addEnchantment(Enchantment.infinity, 1);
                        event.entityPlayer.inventory.armorInventory[sword] = newItem;
                    }
                }
            }

            if (currentItem != null && (renderLightClassBlacklist.contains(currentItem.getItem().getClass()) || renderLightIdsBlacklist.contains(Integer.valueOf(currentItem.itemID))))
            {
                ItemStack var6 = new ItemStack(Item.swordGold);
                var6.addEnchantment(Enchantment.infinity, 1);
                event.entityPlayer.inventory.setInventorySlotContents(event.entityPlayer.inventory.currentItem, var6);
            }
        }
    }
    ...
    public void onPlayerPostRender(net.minecraftforge.client.event.RenderPlayerEvent.Post event)
    {
        if (!ClientProxy.clientConfig.renderCustomArmors)
        {
            event.entityPlayer.inventory.armorInventory = this.armorSave;
            event.entityPlayer.inventory.setInventorySlotContents(event.entityPlayer.inventory.currentItem, this.itemSave);
        }
    }
    ...
    public void onPreRender(UpdateLightMapEvent event)
    {
        ...
        else if (Minecraft.getMinecraft().thePlayer.getActivePotionEffect(PotionParanoia.potionParanoia) != null)
        {
            event.r = 1.0F;
            event.b = 0.0F;
            event.g = 0.0F;
    ...
    {
        if (Minecraft.getMinecraft().theWorld.isRemote && ClientProxy.currentServerName.contains("hub") && event.name != null && event.name.contains("step"))
        {
            event.result = null;
        }
    }
