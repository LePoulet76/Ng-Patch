...
extends ItemBlock {
    ...
    public boolean func_77648_a(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        ...
        if (!stack.func_77942_o()) {
            if (world.field_72995_K) {
                player.func_71035_c("[NationsGUI] " + StatCollector.func_74838_a((String)"nationsgui.radio.select_radio"));
            }
            return false;
        }
        ...
        if (!NationsGUI.SPEAKER.func_71930_b(world, x, y, z)) {
            return false;
        }
        NBTTagCompound compound = stack.func_77978_p();
        int radioX = compound.func_74762_e("RadioX");
        int blockID = world.func_72798_a(radioX, radioY = compound.func_74762_e("RadioY"), radioZ = compound.func_74762_e("RadioZ"));
        ...
        if (world.field_72995_K) {
            player.func_71035_c("[NationsGUI] " + StatCollector.func_74838_a((String)"nationsgui.radio.invalid_location"));
        }
        return false;
    }
    ...
    public void func_77624_a(ItemStack stack, EntityPlayer player, List list, boolean advanced) {
        if (stack.func_77942_o()) {
            ...
            String text = StatCollector.func_74838_a((String)"tile.radio.name") + ": " + x + ", " + y + ", " + z;
            if (blockID != NationsGUI.RADIO.field_71990_ca) {
                text = text + EnumChatFormatting.RED + " (" + StatCollector.func_74838_a((String)"nationsgui.radio.invalid") + ")";
            }
            list.add(text);
        }
    }
