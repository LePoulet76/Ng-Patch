...
extends BlockContainer {
    ...
    public boolean func_71903_a(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ) {
        ...
        if (!world.field_72995_K && ((blockEntity = (IncubatorBlockEntity)world.func_72796_p(x, y, z)).getOwner() == null || blockEntity.getOwner().equals(entityPlayer.field_71092_bJ) || MinecraftServer.func_71276_C().func_71203_ab().func_72353_e(entityPlayer.field_71092_bJ))) {
            entityPlayer.openGui((Object)NationsGUI.INSTANCE, this.field_71990_ca, world, x, y, z);
        }
        return true;
    }
    ...
    public boolean removeBlockByPlayer(World world, EntityPlayer entityPlayer, int x, int y, int z) {
        if (!world.field_72995_K) {
            ...
            ServerConfigurationManager configurationManager = server.func_71203_ab();
            boolean opped = configurationManager.func_72353_e(entityPlayer.field_71092_bJ);
            boolean bl = owner = blockEntity.getOwner() == null || blockEntity.getOwner().equals(entityPlayer.field_71092_bJ);
            return opped ? world.func_94571_i(x, y, z) : owner && world.func_94571_i(x, y, z);
        }
        IncubatorBlockEntity blockEntity = (IncubatorBlockEntity)world.func_72796_p(x, y, z);
        ...
        if (blockEntity != null) {
            for (int j1 = 0; j1 < blockEntity.func_70302_i_(); ++j1) {
                ...
                while (itemstack.field_77994_a > 0) {
                    ...
                    float radius = 0.05f;
                    entityItem.field_70159_w = (float)this.random.nextGaussian() * radius;
                    entityItem.field_70181_x = (float)this.random.nextGaussian() * radius + 0.2f;
                    entityItem.field_70179_y = (float)this.random.nextGaussian() * radius;
    ...
    public void func_71862_a(World world, int x, int y, int z, Random random) {
        ...
        if (blockEntity.isActive()) {
            ...
            world.func_72869_a("smoke", (double)posX, (double)posY, (double)posZ, 0.0, 0.0, 0.0);
            world.func_72869_a("smoke", (double)posX, (double)posY, (double)posZ, 0.0, 0.0, 0.0);
        }
    }
