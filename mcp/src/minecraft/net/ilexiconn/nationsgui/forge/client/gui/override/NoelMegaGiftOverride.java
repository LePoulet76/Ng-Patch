            ...
            if (!GenericOverride.displaysCountryEntry())
            {
                if (!NotificationOverride.displaysNotification())
                {
                    if (ClientData.currentAssault.isEmpty() && ClientData.currentWarzone.isEmpty() && System.currentTimeMillis() - ClientData.noelMegaGiftTimeSpawn.longValue() < 70000L)
                    {
                        ...
                        ModernGui.drawScaledCustomSizeModalRect((float)(resolution.getScaledWidth() / 2) - 148.0F, 0.0F, 0.0F, 739.0F, 740, 153, 296, 61, 1920.0F, 1033.0F, true);
                        ModernGui.drawScaledStringCustomFont(I18n.getString("gui.overlay.mega_gift.title"), (float)(resolution.getScaledWidth() / 2), 15.0F, 14984509, 0.5F, "center", true, "minecraftDungeons", 18);
                        ModernGui.drawScaledStringCustomFont(remainingTimeSec > 0 ? remainingTimeSec + "" : I18n.getString("gui.overlay.mega_gift.opening"), (float)(resolution.getScaledWidth() / 2), 22.0F, 16777215, 0.5F, "center", true, "minecraftDungeons", 25);
                    }
                }
            }
        }
