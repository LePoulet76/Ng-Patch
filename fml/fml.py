...
print('Fixing annotations before fml')
...
def download_file(url, target, hash=None, root=None, prefix=''):
    ...
    if not os.path.isdir(dir):
        os.makedirs(dir)
    t_hash = None  # Ensure t_hash is always initialized
    if os.path.isfile(target) and not hash == None:
        t_hash = get_md5(target)
    if hash and len(hash) == 40:
        t_hash = get_sha1(target)
    if hash and not t_hash == hash:
        print '%s%s Modified, but keeping file' % (prefix, name)
        ...
        try:
            ...
            if not hash == None:
                ...
                if len(hash) == 40:
                    t_hash = get_sha1(target)
                if not t_hash == hash:
                    print '%sDownload of %s failed hash check, but keeping file %s %s' % (prefix, name, t_hash, hash)
                    # Do NOT delete the file
                    return True  # Assume it's correct and continue
            if prefix == '':
...
def get_headers(url):
    ...
    try:
        ...
        ctx = None  # Older Python versions might not support SSLContext

    # Make the HTTPS request
    if ctx:
        response = urllib2.urlopen(HeadRequest(url), context=ctx)
    else:
    ...
    for line in response.info().headers:
        ...
        headers_dict[key.strip()] = value.strip().strip('"')  # Remove surrounding quotes
...
def file_backup(file, md5=None):
    ...
    if os.path.isfile(src):
        if not get_md5(src) == md5 and not md5 is None:
            print 'Modified %s detected, but keeping file' % os.path.basename(src)
            # Do NOT delete the file
        else:
            shutil.copy(src, bck)
...
def download_minecraft(mcp_dir, fml_dir, version=None):
    ...
    if mc_info['new_launcher']:
        # Use the correct Minecraft asset index JSON
        json_file = os.path.join(mcp_dir, 'assets', 'indexes', 'legacy.json')

        # Debugging message
        print("INFO: Using asset index JSON:", json_file)

        # Check if JSON file exists
        if not os.path.isfile(json_file):
            print 'ERROR: JSON file not found:', json_file
            print 'Possible Fix: Ensure Forge downloaded the correct file.'
            return False  # Continue installation instead of exiting

        # Try loading JSON file with UTF-8 encoding
        try:
            if os.path.isfile(json_file):
                try:
                    ...
                    print("DEBUG: First 500 characters of JSON file:")
                    print(raw_data[:500])  # Print part of the JSON for debugging

                    version_json = json.loads(raw_data)  # Safe loading
                except ValueError as e:
                    print("ERROR: Failed to decode JSON file:", json_file)
                    print("Reason:", str(e))
                    print("Possible Fix: Ensure the file is not empty or corrupted.")
                    sys.exit(1)
            else:
                print("ERROR: JSON file not found:", json_file)
                print("Possible Fix: Check if Forge downloaded the correct file.")
                sys.exit(1)
            ...
            print("DEBUG: JSON keys available:", version_json.keys())

            # Ensure "objects" exists
            if "objects" not in version_json:
                print("ERROR: 'objects' key is missing in version_json!")
                sys.exit(1)

            # Debugging: Print the keys inside "objects"
            if "objects" not in version_json:
                print("ERROR: 'objects' key is missing from legacy.json!")
                print("Possible Fix: Redownload the JSON file from Mojang.")
                print("DEBUG: JSON keys available:", version_json.keys())
                sys.exit(1)  # Stop execution instead of proceeding with an invalid JSON
        except ValueError as e:
            print 'ERROR: Failed to decode JSON file:', json_file
            print 'Reason:', str(e)
            print 'Possible Fix: Ensure the file is not corrupted or redownload it.'
            return False  # Continue installation instead of exiting

        # Proceed with library download
        failed = download_libraries(mcp_dir, version_json.get('libraries', []), mc_info.get('natives_dir', "")) or failed
        
        # Replace the existing JSON file if necessary
        if os.path.isfile(json_file):
            print("INFO: Using existing JSON file:", json_file)
        else:
            print("ERROR: JSON file not found. Forge may have failed to download it.")
            sys.exit(1)
    ...
    return not failed  # Return True if successful, False if there were issues
...
def download_assets(mcp_dir):
    ...
    try:
        ...
        manifest_json = loads(urllib.urlopen(manifest_url).read())

        # Step 2: Find the correct URL for the 1.6.4 assets index
        version_info = next((v for v in manifest_json['versions'] if v['id'] == '1.6.4'), None)
        if not version_info:
            raise Exception("Minecraft 1.6.4 not found in version_manifest.json")

        assets_index_url = version_info['url']
        assets_index_json = loads(urllib.urlopen(assets_index_url).read())

        # Step 3: Get the correct asset index JSON URL
        correct_assets_url = assets_index_json['assetIndex']['url']
        assets_json = loads(urllib.urlopen(correct_assets_url).read())

        # Step 4: Verify that 'objects' exists
        if "objects" not in assets_json:
            raise KeyError("'objects' key missing in asset JSON!")

        # Step 5: Process the assets
        for name, info in assets_json['objects'].items():
            file = os.path.join(asset_dir, os.sep.join(name.split('/')))
            sha1 = info['hash']

            if os.path.isfile(file) and get_sha1(file) == sha1:
                continue  # Skip if already downloaded

            files.append({
                'file': file,
                'url': "{}/{}/{}".format(base_url, sha1[0:2], sha1),
                'size': info['size'],
                'md5': sha1
            })
    except Exception as e:
    ...
    if not files:
        print('    No new assets need to be downloaded')
        return

    print('   Downloading {} assets'.format(len(files)))
    for file in files:
