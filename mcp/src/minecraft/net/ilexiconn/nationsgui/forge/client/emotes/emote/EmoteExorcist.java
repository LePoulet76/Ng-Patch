...
public class EmoteExorcist extends EmoteBase
{
    ...
    {
        Timeline timeline = Timeline.createParallel().push(Tween.to(model, 1, 7500.0F).target(((float)Math.PI * 2F))).push((Tween)Tween.to(model, 0, 750.0F).target(-0.05F).repeatYoyo(9, 0.0F));
        return timeline;
    }
