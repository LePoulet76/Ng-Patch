...
public class AuctionFilterSelectorGui<T extends Object> extends Gui
{
    ...
    public void draw(int mouseX, int mouseY)
    {
        ...
        fontRenderer.drawString(text, this.posX + (this.width - fontRenderer.getStringWidth(text)) / 2, this.posY + 4, 16777215);

        if (this.open)
        {
            int i = 1;
            drawRect(this.posX - 1, this.posY + 15, this.posX + this.width + 16, this.posY + (this.stringComparatorMap.size() + 1) * 15 + 1, -13487566);
            ...
            for (Iterator var6 = this.stringComparatorMap.entrySet().iterator(); var6.hasNext(); ++i)
            {
                ...
                drawRect(this.posX, y, this.posX + this.width + 15, y + 15, color);
                fontRenderer.drawString((String)entry.getKey(), this.posX + (this.width - fontRenderer.getStringWidth((String)entry.getKey()) + 15) / 2, y + 2, 16777215);
            }
        }
    }
        ...
        else if (this.open)
        {
            ...
            for (Iterator var4 = this.stringComparatorMap.entrySet().iterator(); var4.hasNext(); ++i)
            {
                ...
                if (mouseX >= this.posX && mouseX <= this.posX + this.width + 15 && mouseY >= y && mouseY <= y + 14)
                {
                    ...
                    PacketDispatcher.sendPacketToServer(PacketRegistry.INSTANCE.generatePacket(new AuctionDataPacket(AuctionGui.search.getText(), (String)entry.getKey(), AuctionGui.currentPage - 1, AuctionGui.myAuctionsOnly)));
                    this.open = false;
                    return true;
                }
            }
