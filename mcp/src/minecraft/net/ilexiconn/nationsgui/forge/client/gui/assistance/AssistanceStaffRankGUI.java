...
public class AssistanceStaffRankGUI extends AbstractAssistanceComponent implements GuiScrollerElement
{
    ...
    {
        float scoreWidth = (float)Minecraft.getMinecraft().fontRenderer.getStringWidth(this.score);
        float maxPseudoWidth = (float)this.width - scoreWidth - 16.0F - 2.0F;
        float pseudoRatio = Math.min(1.0F, maxPseudoWidth / (float)Minecraft.getMinecraft().fontRenderer.getStringWidth(this.pseudo));
        ResourceLocation resourceLocation = AbstractClientPlayer.getLocationSkin(this.pseudo);
        AbstractClientPlayer.getDownloadImageSkin(resourceLocation, this.pseudo);
        ...
        this.drawString(Minecraft.getMinecraft().fontRenderer, this.pseudo, 0, 0, 16777215);
        GL11.glPopMatrix();
        this.drawString(Minecraft.getMinecraft().fontRenderer, this.score, (int)((float)this.width - scoreWidth), 4, 16777215);
    }
